...
class GatewaySessionExpiryRuntimeMixin:
        ...
        while self._running:
            try:
                ...
                for key, entry in expired_entries:
                    try:
                        self._finalize_expired_session(key, entry)
                ...
                self._sweep_idle_and_prune_sessions()
            except Exception as exc:
    ...
    def _finalize_expired_session(self, key: str, entry) -> None:
        try:
    ...
    def _sweep_idle_and_prune_sessions(self) -> None:
        try:
            idle_evicted = self._sweep_idle_cached_agents()
            if idle_evicted:
                logger.info("Agent cache idle sweep: evicted %d agent(s)", idle_evicted)
        except Exception as exc:
            logger.debug("Idle agent sweep failed: %s", exc)

        last_prune_ts = getattr(self, "_last_session_store_prune_ts", 0.0)
        ...
        try:
            ...
            if max_age > 0:
                pruned = self.session_store.prune_old_entries(max_age)
                if pruned:
