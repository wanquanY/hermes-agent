...
class StreamConsumerDeliveryMixin:
    ...
    async def _send_fallback_final(self, text: str) -> None:
        ...
        if not continuation.strip():
            # Telegram can retain only a partial or non-durable streaming
            # preview after a failed final edit. Opt-in adapters commit the
            # completed answer with a fresh final send instead of trusting an
            # empty continuation as proof of durable delivery.
            if (
                final_text.strip()
                ...
                if delivery == "ambiguous":
                    # A timeout may mean the platform accepted the send. Keep
                    # duplicate suppression for that uncertain outcome.
                    self._final_content_delivered = True
                else:
                    # A confirmed failure leaves normal final delivery free to
                    # retry the complete response.
    ...
    async def _send_empty_fallback_final(self, final_text: str) -> str:
        """Commit a completed answer after final streaming edit failure.

        Returns ``delivered`` on confirmed success, ``failed`` when normal
        gateway delivery may safely retry, and ``ambiguous`` when a timeout
        could have reached the platform already.
        """
    ...
    def _send_failure_may_have_delivered(result_or_exc: Any) -> bool:
        ...
        if getattr(result_or_exc, "retryable", None) is True:
            return False
        error = str(
            getattr(result_or_exc, "error", None) or result_or_exc
        ).lower()
