#!/usr/bin/env python3
"""Dovie-owned document parsing tool for Hermes local attachments."""
...
DOVIE_MINERU_PROXY_TIMEOUT_SECONDS = 600
...
class MinerUError(RuntimeError):
    """Raised when the Dovie MinerU proxy is unavailable or returns an error."""
...
def _multipart_body(fields: dict[str, str], files: dict[str, tuple[str, bytes, str]]) -> tuple[bytes, str]:
    boundary = f"dovie-hermes-{hashlib.sha256(os.urandom(16)).hexdigest()}"
    chunks: list[bytes] = []
...
def _http_post_multipart(
    ...
    *,
    ...
    try:
        ...
        raise MinerUError(f"Dovie MinerU proxy request failed: {exc}") from exc

    try:
        result = json.loads(raw.decode("utf-8"))
    except Exception as exc:
        raise MinerUError(f"Dovie MinerU proxy returned non-JSON response: HTTP {status}") from exc
...
def _dovie_mineru_proxy_url() -> str:
    return str(os.getenv("DOVIE_MINERU_PROXY_URL") or "").strip()


def _dovie_runtime_token() -> str:
    return str(os.getenv("DOVIE_LLM_RUNTIME_TOKEN") or "").strip()


def _parse_with_dovie_mineru_proxy(
    *,
    path: Path | None,
    file_url: str,
    file_type: str,
    ...
    proxy_url = _dovie_mineru_proxy_url()
    token = _dovie_runtime_token()
    if not proxy_url:
        raise MinerUError("Dovie MinerU proxy is not configured")
    if not token:
        raise MinerUError("Dovie runtime token is not configured")
    ...
    result = _http_post_multipart(
        ...
        files=files,
        timeout=_env_int("DOVIE_MINERU_PROXY_TIMEOUT", DOVIE_MINERU_PROXY_TIMEOUT_SECONDS),
    )
    return {
...
def parse_document_tool(
    ...
    if ext in MINERU_SUPPORTED_EXTENSIONS:
        try:
            parsed = _parse_with_dovie_mineru_proxy(
                path=resolved,
            ...
            logger.warning(
                "parse_document: Dovie MinerU proxy failed file_type=%s file_name=%s fallback=%s error=%s",
                ext,
...
DOCUMENT_PARSE_SCHEMA = {
    ...
    "description": (
        ...
        "use the Dovie-managed MinerU proxy when available, with safe local "
