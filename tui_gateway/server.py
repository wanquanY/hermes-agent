...
def _set_session_context(session_key: str) -> list:
    try:
        ...
        return set_session_vars(
            ...
            terminal_cwd=str(session.get("cwd") or ""),
            doxie_product_context=str(session.get("doxie_product_context") or ""),
        )
    except Exception:
        return []
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        cwd = normalize_session_cwd(params.get("cwd") or cwd)
        workspace = workspace_from_params(params, cwd)
        workspace = bind_session_workspace(
            session_id=key,
            cwd=cwd,
            workspace=workspace,
        )
    ...
    _sessions[sid] = {
        ...
        "doxie_product_context": json.dumps(
            params.get("doxieProductContext") or params.get("doxie_product_context") or {},
            ensure_ascii=False,
        )
        if isinstance(params.get("doxieProductContext") or params.get("doxie_product_context"), dict)
        else str(params.get("doxieProductContext") or params.get("doxie_product_context") or ""),
        "edit_snapshots": {},
        "history": [],
        "history_lock": threading.Lock(),
        "history_version": 0,
...
def _(rid, params: dict) -> dict:
    try:
        ...
        return _ok(rid, manage_cron(params or {}))
    except Exception as e:
...
def _(rid, params: dict) -> dict:
    try:
        ...
        stored_session_id = str(session.get("session_key") or params.get("stored_session_id") or "").strip()
        if stored_session_id:
            from tui_gateway.services.workspace import workspace_for_session
        ...
        return _ok(rid, {"workspace": workspace or None})
    except Exception as e:
        return _err(rid, 5034, str(e))
...
def _(rid, params: dict) -> dict:
    try:
        from tui_gateway.services.workspace import list_workspaces

        limit = max(1, min(500, int(params.get("limit") or 200)))
