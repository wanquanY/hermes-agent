...
def _profile_cron_storage_scope():
    ...
    cron.jobs predates Dovie's multi-profile control plane and keeps storage
...
def cron_status() -> dict[str, Any]:
    ...
    from tui_gateway.services.dovie_cron_runtime import cron_ticker_status
...
def add_cron_job(params: dict[str, Any]) -> dict[str, Any]:
    ...
    from tui_gateway.services.dovie_cron_runtime import request_cron_tick
    ...
    updated = update_job(job["id"], {
        ...
        "dovie": _dovie_metadata_from_params(params),
    }) or job
    if not _bool(params.get("enabled"), default=True):
        updated = pause_job(job["id"], reason="disabled from Dovie") or updated
...
def update_cron_job(params: dict[str, Any]) -> dict[str, Any]:
    ...
    existing_dovie = current.get("dovie") if isinstance(current.get("dovie"), dict) else {}
    dovie_patch = _dovie_metadata_from_params(patch, partial=True)
    dovie_patch = {
        k: v
        for k, v in dovie_patch.items()
        if v is not None or k == "capability_override"
    }
    updates["dovie"] = {**existing_dovie, **dovie_patch}

    updated = update_job(job_id, updates) if updates else current
    if "enabled" in patch:
        updated = (resume_job(job_id) if _bool(patch.get("enabled"), default=True) else pause_job(job_id, reason="disabled from Dovie")) or updated
    return _normalize_job(updated)
...
def toggle_cron_job(params: dict[str, Any], *, enabled: bool) -> dict[str, Any]:
    ...
    job = resume_job(_job_id(params)) if enabled else pause_job(_job_id(params), reason="disabled from Dovie")
    if not job:
...
def _normalize_job(job: dict[str, Any]) -> dict[str, Any]:
    ...
    owner = _owner_from_dovie(job, dovie)
    result_binding = _result_binding_from_dovie(job, dovie)
    target_session_id = _result_binding_session_id(result_binding) or _optional_text(dovie.get("session_id"))
    payload_kind = _payload_kind_from_dovie(job, dovie)
    session_target = "main" if result_binding.get("mode") == "current-session" else "isolated"
    capability = _capability_projection(job, dovie)
    return {
        "id": job_id,
        "jobId": job_id,
        "name": _text(job.get("name") or job_id or "自动化任务"),
        "description": _optional_text(job.get("description") or dovie.get("description")),
        "enabled": _bool(job.get("enabled"), default=True),
        "deleteAfterRun": _bool(dovie.get("delete_after_run"), default=False),
        "agentProfileId": _optional_text(owner.get("agentProfileId")),
        "agentProfileName": _optional_text(owner.get("agentProfileName")),
        "approvalPolicy": _normalize_approval_policy(dovie.get("approval_policy")),
        ...
        "wakeMode": _optional_text(dovie.get("wake_mode")) or "next-heartbeat",
        "origin": _created_by_origin(dovie.get("created_by") or owner.get("createdBy")),
        "owner": owner,
    ...
    if output_dir.exists():
        for file_path in output_dir.glob("*.md"):
            ...
            entries.append({
                ...
                "targetSessionId": _dovie_session_id(job),
                "runAtMs": ts_ms,
                "nextRunAtMs": _iso_to_ms(job.get("next_run_at")),
                "model": _optional_text(job.get("model")),
                "provider": _optional_text(job.get("provider")),
...
def _owner_from_dovie(job: dict[str, Any], dovie: dict[str, Any]) -> dict[str, Any]:
    owner = dovie.get("owner") if isinstance(dovie.get("owner"), dict) else {}
    merged = {
        "agentProfileId": owner.get("agentProfileId") or owner.get("agent_profile_id") or dovie.get("agent_profile_id"),
        "agentProfileName": owner.get("agentProfileName") or owner.get("agent_profile_name") or dovie.get("agent_profile_name"),
        ...
        "workspaceId": owner.get("workspaceId") or owner.get("workspace_id") or dovie.get("workspace_id"),
        "workdir": owner.get("workdir") or dovie.get("workdir") or job.get("workdir"),
        ...
        "createdBy": _created_by_origin(owner.get("createdBy") or owner.get("created_by") or dovie.get("created_by")),
    }
...
def _result_binding_from_dovie(job: dict[str, Any], dovie: dict[str, Any]) -> dict[str, Any]:
    ...
    if binding:
        ...
        if mode == "current-session":
            ...
            if deliver:
                result["deliver"] = deliver
        return result
    if _optional_text(dovie.get("session_target")) == "main":
        return {"mode": "current-session", "sessionId": _optional_text(dovie.get("session_id")) or ""}
    if _optional_text(job.get("deliver")):
        return {"mode": "hermes-native", "deliver": _optional_text(job.get("deliver"))}
    return {"mode": "new-session"}


def _payload_kind_from_dovie(job: dict[str, Any], dovie: dict[str, Any]) -> str:
...
def _capability_projection(job: dict[str, Any], dovie: dict[str, Any]) -> dict[str, Any]:
    override = dovie.get("capability_override") if isinstance(dovie.get("capability_override"), dict) else {}
...
def _created_by_origin(value: Any) -> str:
    ...
    if text in {"manual", "conversation", "hermes-native"}:
        return text
    if text == "dovie-desktop":
        return "manual"
    return "unknown"


def _dovie_metadata_from_params(params: dict[str, Any], *, partial: bool = False) -> dict[str, Any]:
...
def _dovie_session_id(job: dict[str, Any]) -> str | None:
    dovie = job.get("dovie") if isinstance(job.get("dovie"), dict) else {}
    return _result_binding_session_id(_result_binding_from_dovie(job, dovie)) or _optional_text(dovie.get("session_id"))


def _cron_run_session_id(job: dict[str, Any]) -> str | None:
    return _optional_text(job.get("last_session_id")) or _dovie_session_id(job)
