...
def _interval_seconds() -> float:
    raw = os.getenv("DOVIE_CRON_TICK_INTERVAL_SECONDS", "").strip()
    if not raw:
...
def start_cron_ticker(*, interval_seconds: float | None = None) -> None:
    """Start the Dovie sidecar-owned Hermes cron ticker.

    Dovie runs Hermes through the lightweight TUI JSON-RPC gateway rather than
    Hermes' full messaging gateway. The full gateway owns its own cron ticker;
    this sidecar must therefore own the scheduler loop for Dovie profile
