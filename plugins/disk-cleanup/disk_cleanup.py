...
def guess_category(path: Path) -> Optional[str]:
    ...
    try:
        ...
        if top == "cron" or top == "cronjobs":
            # Canonical output lives under ``output/``. Older Hermes builds
            # also wrote individual output files directly under ``cron/``;
            # retain that cleanup compatibility while protecting the exact
            # control-plane paths (jobs.json, .tick.lock, cron/output itself).
