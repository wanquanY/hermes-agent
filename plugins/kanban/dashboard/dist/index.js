...
(function () {
  ...
  function InlineCreate(props) {
    ...
    const submit = function () {
      ...
      if (goalMode) {
        body.goal_mode = true;
        const parsedGoalMaxTurns = parseInt(goalMaxTurns, 10);
        if (Number.isFinite(parsedGoalMaxTurns) && parsedGoalMaxTurns > 0) {
          body.goal_max_turns = parsedGoalMaxTurns;
        }
      }
      props.onSubmit(body);
    ...
    return h("div", { className: "hermes-kanban-inline-create" },
      ...
      h("div", { className: "flex gap-2 items-center" },
        h("label", {
          ...
          title: "Keep this worker in a judge-driven goal loop until the card is done or externally blocked.",
        },
          h("input", {
            type: "checkbox",
            checked: goalMode,
            onChange: function (event) { setGoalMode(!!event.target.checked); },
        ...
        goalMode ? h(Input, {
          type: "number",
          value: goalMaxTurns,
          onChange: function (event) { setGoalMaxTurns(event.target.value); },
