...
All three are sync — the underlying call is ``httpx.post(...)``.
...
Tavily requires Bearer authentication as well as the body API key for
``/crawl``; ``/search`` and ``/extract`` use the body API key.
"""
...
class TavilyWebSearchProvider(WebSearchProvider):
    ...
    def crawl(self, url: str, **kwargs: Any) -> Dict[str, Any]:
        """Crawl a seed URL through Tavily's native ``/crawl`` endpoint."""
        try:
            from tools.interrupt import is_interrupted
