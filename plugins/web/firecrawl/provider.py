...
class FirecrawlWebSearchProvider(WebSearchProvider):
    """Firecrawl search + extract + crawl provider with dual auth paths."""
    ...
    async def crawl(self, url: str, **kwargs: Any) -> Dict[str, Any]:
        """Crawl a seed URL through Firecrawl's native ``/crawl`` endpoint.

        The SDK call runs in a worker thread. Both the seed URL and every URL
        returned by Firecrawl are checked against Hermes' SSRF and website
        policy boundaries, so redirects or provider-side discovery cannot
        reintroduce an internal-network fetch into the local result stream.
        """
        try:
            from tools.interrupt import is_interrupted
            ...
            if instructions:
                logger.info(
                    "Firecrawl crawl ignores instructions because /crawl "
                    "does not accept natural-language guidance"
                )
            ...
            if hasattr(crawl_result, "data"):
                data_list = getattr(crawl_result, "data", None) or []
            elif isinstance(crawl_result, dict):
                data_list = crawl_result.get("data", []) or []
            else:
                data_list = []
                logger.warning(
                    "Firecrawl crawl returned unexpected type %s",
            ...
            for item in data_list:
                ...
                content = plain.get("markdown") or plain.get("html") or ""
                pages.append(
                    {
                        "url": page_url,
                        "title": title,
