...
def _resolve_model(explicit: Optional[str] = None) -> Tuple[str, Dict[str, Any]]:
    ...
    env_override = get_profile_env("KREA_IMAGE_MODEL")
...
def _resolve_managed_krea_gateway():
    ...
    if get_profile_env("KREA_API_KEY") and not prefers_gateway("image_gen"):
        return None
...
class KreaImageGenProvider(ImageGenProvider):
    ...
    def is_available(self) -> bool:
        ...
        return bool(get_profile_env("KREA_API_KEY")) or _managed_krea_gateway_ready()
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if managed is not None:
            ...
            base_url = BASE_URL
            auth_token = get_profile_env("KREA_API_KEY")
            if not auth_token:
                return error_response(
                    error=(
