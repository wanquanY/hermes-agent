...
def _resolve_model(catalog: List[Dict[str, Any]], cfg: Dict[str, Any]) -> Optional[str]:
    ...
    """
    env_override = get_profile_env("DEEPINFRA_IMAGE_MODEL", "").strip()
    if env_override:
        return env_override
...
class DeepInfraImageGenProvider(ImageGenProvider):
    ...
    def is_available(self) -> bool:
        return bool(get_profile_env("DEEPINFRA_API_KEY", "").strip())
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        api_key = get_profile_env("DEEPINFRA_API_KEY", "").strip()
        if not api_key:
            return error_response(
                error=(
