...
- Self-hosted: a running [Supermemory local](https://supermemory.ai/docs/self-hosting/overview) server and its API key
...
For a self-hosted instance, configure the endpoint before running setup so
the setup probe also stays local:
...
|-----|---------|-------------|
| `base_url` | `https://api.supermemory.ai` | Hosted or self-hosted endpoint. Overrides `SUPERMEMORY_BASE_URL`. |
...
| `SUPERMEMORY_BASE_URL` | Endpoint fallback when `base_url` is not configured |
| `SUPERMEMORY_CONTAINER_TAG` | Override container tag (takes priority over config file) |

Endpoint precedence is `supermemory.json` → `SUPERMEMORY_BASE_URL` →
`https://api.supermemory.ai`. The resolved endpoint is shared by SDK calls,
setup/status probes, and full-session conversation ingest.
...
- route every API path through the same hosted or self-hosted endpoint
