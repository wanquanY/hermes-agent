...
def _resolve_base_url(config_value: Any = "") -> str:
    """Resolve one endpoint for SDK, setup probes, and conversation ingest."""
    configured = str(config_value or "").strip()
    environment = os.environ.get("SUPERMEMORY_BASE_URL", "").strip()
    return (configured or environment or _DEFAULT_BASE_URL).rstrip("/")
