...
def _session_item(db: SessionDB, session_id: str = "stored-session-1") -> dict:
    rows = {
        item["session_id"]: item
        for item in db.list_session_index()["sessions"]
    }
...
def test_session_index_includes_active_activity_count_for_pending_and_running(
    ...
    db.update_activity_status("running", "running")
    db.update_activity_status("other-conv", "running")
...
def test_session_index_includes_unread_completion_count_for_completed_failed_with_read_null(
    ...
    db.mark_activity_completed("completed", result_summary="done", result_json={})
    db.mark_activity_failed("failed", error_message="failed")
...
def test_session_index_aggregates_across_multiple_activities_per_conv(tmp_path: Path) -> None:
    ...
    db.mark_activity_completed("completed", result_summary="done", result_json={})
    db.mark_activity_failed("failed", error_message="failed")
    db.mark_activity_completed("read", result_summary="done", result_json={})
    db.mark_activity_read("read")
    db.mark_activity_cancelled("cancelled")
