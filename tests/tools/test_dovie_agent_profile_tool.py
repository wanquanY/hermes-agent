...
import tools.dovie_agent_profile_tool as dovie_agent_profile_tool
from tools.dovie_agent_profile_tool import (
...
def test_design_agent_profile_inspects_real_design_context():
    ...
    assert result["dovie_event"] == "agent_profile_design_context"
    assert result["catalogKind"] == "overview"
    assert "web" in result["systemToolsets"]
    assert "dovie" not in result["systemToolsets"]
...
def test_design_agent_profile_inspects_catalog_slice_on_demand():
    ...
    assert all(item["name"] != "dovie" for item in result["catalog"])
...
def test_design_agent_profile_emits_composite_design_event():
    ...
    assert result["dovie_event"] == "agent_profile_design_draft_requested"
...
def test_design_agent_profile_saves_draft_through_backend_bridge(monkeypatch):
    ...
    monkeypatch.setattr("tools.dovie_agent_profile_tool._backend_call", fake_backend_call)
    result = json.loads(
        design_agent_profile(
            operation="create",
            name="产品经理分身",
    ...
    assert calls[0][0] == "dovie_agent_profile_draft_create"
    assert calls[0][1]["name"] == "产品经理分身"
    assert calls[0][1]["category"] == "工作"
    assert result["dovie_event"] == "agent_profile_design_draft_saved"
...
def test_backend_bridge_call_returns_on_interrupt(monkeypatch):
    ...
    monkeypatch.setattr(dovie_agent_profile_tool.urllib.request, "urlopen", fake_urlopen)

    def run_call():
        try:
            dovie_agent_profile_tool._backend_call("dovie_agent_profile_draft_get", {"draftId": "draft-1"})
    ...
    assert result_holder["error"] == "Dovie backend bridge request interrupted"
...
def test_design_agent_profile_creates_revision_from_selected_target(monkeypatch):
    ...
    monkeypatch.setenv("DOVIE_BACKEND_BRIDGE_TOKEN", "token")
    monkeypatch.setattr(
        "tools.dovie_agent_profile_tool._session_design_context",
    ...
    def fake_backend_call(command, payload):
        calls.append((command, payload))
        assert command == "dovie_agent_profile_draft_create_revision"
        return {
            "id": "draft-revision-1",
            "draftKind": "revision",
    ...
    assert calls[0][0] == "dovie_agent_profile_draft_create_revision"
...
def test_install_skill_to_agent_profile_draft_copies_skill_and_updates_draft(monkeypatch, tmp_path):
    ...
    def fake_backend_call(command, payload):
        calls.append((command, payload))
        if command == "dovie_agent_profile_draft_prepare_runtime":
            return {
                "prepared": True,
                "draft": {
                    "id": "draft-1",
        ...
        if command == "dovie_agent_profile_draft_update":
            return {
                "id": payload["draftId"],
    ...
    assert result["dovie_event"] == "agent_profile_draft_skill_installed"
    assert result["draftId"] == "draft-1"
    ...
    assert calls[-1][0] == "dovie_agent_profile_draft_update"
...
def test_test_agent_profile_runs_draft_through_delegation(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("tools.dovie_agent_profile_tool._draft_home", lambda _draft_id: draft_home)
    ...
    assert result["dovie_event"] == "agent_profile_test_completed"
    assert result["draftId"] == "draft-1"
