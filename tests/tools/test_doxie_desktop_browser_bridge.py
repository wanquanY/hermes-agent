...
def test_doxie_browser_bridge_uses_desktop_browser_commands(monkeypatch):
    ...
    assert snapshot["provider"] == "doxie_desktop"
...
def test_browser_actions_prefer_doxie_desktop_bridge(monkeypatch):
    ...
    assert click_payload["provider"] == "doxie_desktop"
    assert type_payload["provider"] == "doxie_desktop"
    assert scroll_payload["provider"] == "doxie_desktop"
    assert press_payload["provider"] == "doxie_desktop"
    assert calls == [
        ("click", {"ref": "@e1"}),
