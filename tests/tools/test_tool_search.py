...
class TestDovieRuntimeScoping:
    """Dovie product contracts stay visible and exact profile filters win."""
    ...
    def test_dovie_and_team_mission_tools_are_never_deferred(self):
        from tools.tool_search import classify_tools

        self._register("dovie_runtime_contract_test", "dovie")
        self._register("team_mission_contract_test", "team_mission_handoff")
        visible, deferred = classify_tools([
            _td("dovie_runtime_contract_test"),
            _td("team_mission_contract_test"),
        ])
        assert {item["function"]["name"] for item in visible} == {
            "dovie_runtime_contract_test",
            "team_mission_contract_test",
        }
        assert deferred == []

    def test_parent_exact_tool_filter_bounds_bridge_dispatch(self):
        from types import SimpleNamespace
        import model_tools

        self._register("profile_exact_allowed", "profile-plugin")
        self._register("profile_exact_denied", "profile-plugin")
        parent = SimpleNamespace(
            enabled_toolsets=None,
            disabled_toolsets=None,
            _enabled_tool_names_filter={"profile_exact_allowed"},
        )

        allowed = json.loads(model_tools.handle_function_call(
            function_name="tool_call",
            function_args={"name": "profile_exact_allowed", "arguments": {}},
            parent_agent=parent,
        ))
        assert allowed == {"ok": True, "tool": "profile_exact_allowed"}

        denied = json.loads(model_tools.handle_function_call(
            function_name="tool_call",
            function_args={"name": "profile_exact_denied", "arguments": {}},
            parent_agent=parent,
        ))
