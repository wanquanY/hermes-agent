...
def test_memory_approve_without_live_agent_uses_fresh_store(hermes_home):
    """Review surfaces can apply memory without owning a live AIAgent."""
    import json
    from hermes_cli.write_approval_commands import handle_pending_subcommand
    from tools.memory_tool import load_on_disk_store, memory_tool, MemoryStore
    from tools import write_approval as wa
...
def test_pending_store_failure_is_not_reported_as_staged(hermes_home, monkeypatch):
    from tools import write_approval as wa

    def fail_replace(*_args):
        raise OSError("disk full")

    monkeypatch.setattr(wa.os, "replace", fail_replace)

    with pytest.raises(RuntimeError, match="durably stage"):
        wa.stage_write(
            "memory",
...
def test_memory_gate_surfaces_staging_failure(hermes_home, monkeypatch):
    from tools import write_approval as wa
    ...
    result = json.loads(memory_tool("add", "memory", "must persist", store=store))
