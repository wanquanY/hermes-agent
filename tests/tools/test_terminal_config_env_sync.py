...
at startup, by THREE separate code paths:
  ...
  2. hermes_gateway/runner.py    -> ``_terminal_env_map`` dict (gateway / messaging
                          platforms)
...
def _gateway_env_map_keys() -> set[str]:
    """terminal config keys bridged by hermes_gateway/runner.py at module load."""
    # hermes_gateway/runner.py builds the dict at module top-level (not inside a
    # function), so inspect the whole module source.
    import hermes_gateway.runner as gr
...
# hermes_gateway/runner.py or set_config_value.  Each entry must be justified.
...
def test_cli_and_gateway_env_maps_agree():
    ...
    assert not missing_in_gateway, (
        f"Keys in cli.py env_mappings but missing from hermes_gateway/runner.py "
    ...
    assert not missing_in_cli, (
        f"Keys in hermes_gateway/runner.py _terminal_env_map but missing from cli.py "
...
def test_docker_run_as_host_user_is_bridged_everywhere():
    ...
    DockerEnvironment but NOT to cli.py's env_mappings or hermes_gateway/runner.py's
...
def test_docker_mount_cwd_to_workspace_is_bridged_everywhere():
    ...
    hermes_gateway/runner.py's _terminal_env_map until the docker_run_as_host_user
    audit caught it.
    """
