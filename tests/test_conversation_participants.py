...
def test_participant_upsert_list_and_get(tmp_path: Path):
    ...
    participants = db.participants.list_conversation_participants("conv")
    ...
    alice = db.participants.get_conversation_participant("conv", "member:m1")
...
def test_participant_upsert_is_idempotent_and_preserves_display(tmp_path: Path):
    ...
    assert len(db.participants.list_conversation_participants("conv")) == 1
    row = db.participants.get_conversation_participant("conv", "member:m1")
...
def test_resolve_participant_for_run_priority(tmp_path: Path):
    ...
    assert db.participants.resolve_participant_id_for_run("conv", member_id="m1") == "member:m1"
    # scope key resolves the running participant.
    assert db.participants.resolve_participant_id_for_run(
        "conv", runtime_scope_key="team:conv:leader-conversation"
    ) == "leader:conv"
    # profile id is unambiguous within a team.
    assert db.participants.resolve_participant_id_for_run("conv", agent_profile_id="p-alice") == "member:m1"
    # nothing matches → empty, caller decides the fallback.
    assert db.participants.resolve_participant_id_for_run("conv", agent_profile_id="ghost") == ""
    # unknown conversation → empty.
    assert db.participants.resolve_participant_id_for_run("other", member_id="m1") == ""
...
def test_run_event_routes_by_conversation_session_id_without_mission(tmp_path: Path):
    ...
    """
    db = open_cli_session_store(tmp_path / "state.db")
    db.sessions.create("plain-conv", source="chat", transient=False)
    db.runs.upsert(run_id="run-x", session_id="plain-conv", status="running")
    ...
    events = db.runs.list_events("plain-conv")
    ...
    by_run = db.runs.list_events("plain-conv", run_id="run-x")
    assert len(by_run) >= 1
...
def test_record_event_stamps_participant_id_for_leader(tmp_path: Path):
    ...
    db = open_cli_session_store(tmp_path / "state.db")
    db.teams.upsert_agent_team(team_id="team-1", name="Team")
    db.teams.upsert_agent_team_member(
        member_id="m-lead", team_id="team-1", agent_profile_id="p-lead",
        role="lead", profile_name="Lead",
    )
    ...
    db.runs.upsert(run_id="run-leader", session_id="team-session-1", status="running")
    ...
    events = db.runs.list_events("team-session-1", run_id="run-leader")
...
def test_record_event_stamps_participant_id_for_member_by_profile(tmp_path: Path):
    ...
    db.runs.upsert(run_id="run-alice", session_id="team-session-1", status="running")
    ...
    events = db.runs.list_events("team-session-1", run_id="run-alice")
    assert events
...
def test_record_event_with_no_hints_does_not_stamp(tmp_path: Path):
    ...
    db = open_cli_session_store(tmp_path / "state.db")
    db.sessions.create("plain-conv", source="chat", transient=False)
    db.runs.upsert(run_id="run-y", session_id="plain-conv", status="running")
    ...
    events = db.runs.list_events("plain-conv", run_id="run-y")
    assert events
