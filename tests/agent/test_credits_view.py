...
def _make_gateway_stub():
    """Minimal local gateway service exposing the top-up handler."""
    from hermes_gateway.topup_command import GatewayTopupCommandService

    return GatewayTopupCommandService(object())
...
def test_gateway_topup_renders_block_and_url(monkeypatch):
    ...
    out = asyncio.run(stub.handle_topup_command(_FakeEvent()))
