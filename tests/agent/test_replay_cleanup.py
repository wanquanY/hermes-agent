...
def test_sanitize_replay_history_expires_stale_dangerous_confirmation():
    history = [
        {
            "role": "user",
    ...
    output = sanitize_replay_history(history, now=200.0)

    assert "EXPIRED" in output[0]["content"]
    assert "api_content" not in output[0]
    assert history[0]["content"] == "confirm forced restart"


def test_stale_confirmation_keeps_fresh_and_untimestamped_messages():
    history = [
        {"role": "user", "content": "confirm shutdown", "timestamp": 190.0},
        {"role": "user", "content": "confirm reboot"},
    ]

    assert strip_stale_dangerous_confirmations(history, now=200.0) == history
