...
class TestAnthropicMcpPrefixStrip:
    ...
    def test_restores_single_underscore_mcp_server_tool(self):
        ...
        """
        transport = self._get_transport()
        block = _make_tool_use_block("mcp__linear__get_issue")
        response = _make_response(block)

        registry = _FakeRegistry({"mcp__linear__get_issue", "read_file"})
        with patch("tools.registry.registry", registry):
            result = transport.normalize_response(response, strip_tool_prefix=True)

        assert len(result.tool_calls) == 1
        assert result.tool_calls[0].name == "mcp__linear__get_issue"
    ...
    def test_mixed_native_and_mcp_server_tools_same_response(self):
        ...
        block2 = _make_tool_use_block("mcp__linear__get_issue", block_id="tc_2")
        response = _make_response(block1, block2)

        registry = _FakeRegistry({"read_file", "mcp__linear__get_issue"})
        with patch("tools.registry.registry", registry):
            result = transport.normalize_response(response, strip_tool_prefix=True)

        assert len(result.tool_calls) == 2
        assert result.tool_calls[0].name == "read_file"
        assert result.tool_calls[1].name == "mcp__linear__get_issue"
