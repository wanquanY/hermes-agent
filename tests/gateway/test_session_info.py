...
def _patch_info(tmp_path, config_yaml, model, runtime):
    ...
    return (
        patch("hermes_constants.get_hermes_home", lambda: tmp_path),
        patch("hermes_gateway.reset_command._resolve_gateway_model", return_value=model),
        patch("hermes_gateway.reset_command._resolve_runtime_agent_kwargs", return_value=runtime),
    )
...
class TestFormatSessionInfo:
    ...
    def test_runtime_resolution_failure_doesnt_crash(self, runner, tmp_path):
        ...
        with patch("hermes_constants.get_hermes_home", lambda: tmp_path), \
             patch("hermes_gateway.reset_command._resolve_gateway_model", return_value="test-model"), \
             patch("hermes_gateway.reset_command._resolve_runtime_agent_kwargs", side_effect=RuntimeError("no creds")):
            info = runner._format_session_info()
        assert "4K" in info
        assert "config" in info
