...
async def test_yolo_command_toggles_only_current_session(monkeypatch):
    ...
    result_on = await gateway_run.yolo_command_for(runner).handle_yolo_command(event_a)
    ...
    result_off = await gateway_run.yolo_command_for(runner).handle_yolo_command(event_a)
