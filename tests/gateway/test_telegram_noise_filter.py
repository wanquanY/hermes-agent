...
from hermes_gateway.output_policy import (
    prepare_gateway_status_message,
...
def test_non_telegram_status_is_unchanged():
    ...
    assert prepare_gateway_status_message(Platform.DISCORD, "lifecycle", message) == message
    assert prepare_gateway_status_message("local", "lifecycle", message) == message
...
def test_telegram_final_response_keeps_normal_answers():
    ...
    assert sanitize_gateway_final_response(Platform.TELEGRAM, answer) == answer
