...
def test_matrix_home_target_env_var_uses_home_room():
    assert home_target_env_var("matrix") == "MATRIX_HOME_ROOM"


def test_email_home_target_env_var_uses_home_address():
    assert home_target_env_var("email") == "EMAIL_HOME_ADDRESS"


def test_telegram_home_target_env_var_uses_home_channel():
    assert home_target_env_var("telegram") == "TELEGRAM_HOME_CHANNEL"


def test_discord_home_target_env_var_uses_home_channel():
    assert home_target_env_var("discord") == "DISCORD_HOME_CHANNEL"


def test_unknown_platform_home_target_env_var_falls_back_to_home_channel():
    assert home_target_env_var("custom") == "CUSTOM_HOME_CHANNEL"
...
def test_home_thread_env_var_uses_home_target_name_plus_thread_id():
    assert home_thread_env_var("discord") == "DISCORD_HOME_CHANNEL_THREAD_ID"
    assert home_thread_env_var("matrix") == "MATRIX_HOME_ROOM_THREAD_ID"
    assert home_thread_env_var("email") == "EMAIL_HOME_ADDRESS_THREAD_ID"
