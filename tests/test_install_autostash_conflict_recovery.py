...
def _make_conflicted_checkout(tmp_path: Path) -> Path:
    seed = tmp_path / "seed"
    seed.mkdir()
    _git(seed, "init")
...
def _assert_recovered(repo: Path, output: str) -> None:
...
def test_install_sh_recovers_from_autostash_conflict(tmp_path: Path) -> None:
    managed = _make_conflicted_checkout(tmp_path)
    ...
    _assert_recovered(managed, result.stdout)
...
def test_install_ps1_recovers_from_autostash_conflict(tmp_path: Path) -> None:
    managed = _make_conflicted_checkout(tmp_path)
    result = subprocess.run(
        [
            str(POWERSHELL),
