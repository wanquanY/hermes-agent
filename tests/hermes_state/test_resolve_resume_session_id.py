...
``CliSessionStore.resolve_resume_session_id()`` walks the parent → child chain
...
def _make_chain(db: CliSessionStore, ids_with_parent):
    ...
    for i, (sid, parent) in enumerate(ids_with_parent):
        db.sessions.create(sid, source="cli", parent_session_id=parent)
        db._conn.execute(
...
def test_redirects_from_empty_head_to_descendant_with_messages(db):
    ...
    for i in range(5):
        db.messages.append("bulk", role="user", content=f"msg {i}")

    assert db.sessions.resolve_resume_id("head") == "bulk"


def test_returns_self_when_session_has_messages(db):
    _make_chain(db, [("root", None), ("child", "root")])
    db.messages.append("root", role="user", content="hi")
    assert db.sessions.resolve_resume_id("root") == "root"
...
def test_returns_self_for_isolated_session(db):
    db.sessions.create("isolated", source="cli")
    assert db.sessions.resolve_resume_id("isolated") == "isolated"


def test_returns_self_for_nonexistent_session(db):
    assert db.sessions.resolve_resume_id("does_not_exist") == "does_not_exist"


def test_empty_session_id_passthrough(db):
    assert db.sessions.resolve_resume_id("") == ""
    assert db.sessions.resolve_resume_id(None) == ""
...
def test_walks_from_middle_of_chain(db):
    ...
    db.messages.append("d", role="user", content="x")
    assert db.sessions.resolve_resume_id("b") == "d"
    assert db.sessions.resolve_resume_id("c") == "d"
...
def test_follows_compression_tip_when_parent_retains_messages(db):
    ...
    db.messages.append("root", role="user", content="pre-compression turn")
    db.sessions.end("root", "compression")
    db.sessions.create("cont", source="cli", parent_session_id="root")
    db.messages.append("cont", role="assistant", content="post-compression reply")
    ...
    assert db.sessions.resolve_resume_id("root") == "cont"
...
def test_compression_tip_not_confused_with_delegation_child(db):
    ...
    base = int(time.time()) - 10_000
    db.sessions.create("conv", source="cli")
    db.messages.append("conv", role="user", content="parent turn")
    db.sessions.create("subagent", source="cli", parent_session_id="conv")
    db.messages.append("subagent", role="assistant", content="delegated work")
    ...
    assert db.sessions.resolve_resume_id("conv") == "conv"
...
def test_prefers_most_recent_child_when_fork_exists(db):
    ...
    db.messages.append("newer_fork", role="user", content="x")
    assert db.sessions.resolve_resume_id("parent") == "newer_fork"
