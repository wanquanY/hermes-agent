"""Tests for CliSessionStore.get_messages_around (anchored-window primitive).
...
def _seed(db, sid="s1", n=10):
    ...
    for i in range(n):
        ...
        mid = db.messages.append(sid, role=role, content=f"msg {i}")
        ids.append(mid)
    return ids
...
class TestBasicWindow:
    def test_returns_window_around_anchor(self, db):
        ids = _seed(db, n=10)
        anchor = ids[5]
        view = db.messages.window_around("s1", anchor, window=2)
    ...
    def test_window_zero_returns_only_anchor(self, db):
        ids = _seed(db, n=5)
        view = db.messages.window_around("s1", ids[2], window=0)
    ...
    def test_negative_window_clamps_to_zero(self, db):
        ids = _seed(db, n=5)
        view = db.messages.window_around("s1", ids[2], window=-3)
...
class TestBoundaryDetection:
    ...
    def test_at_session_start_messages_before_is_short(self, db):
        ...
        view = db.messages.window_around("s1", ids[0], window=5)
    ...
    def test_at_session_end_messages_after_is_short(self, db):
        ids = _seed(db, n=10)
        view = db.messages.window_around("s1", ids[-1], window=5)
    ...
    def test_window_larger_than_session(self, db):
        ids = _seed(db, n=3)
        view = db.messages.window_around("s1", ids[1], window=50)
...
class TestAnchorValidation:
    def test_missing_anchor_returns_empty(self, db):
        _seed(db, n=5)
        view = db.messages.window_around("s1", 99999, window=5)
        assert view["window"] == []
        assert view["messages_before"] == 0
        assert view["messages_after"] == 0
    ...
    def test_anchor_in_different_session_returns_empty(self, db):
        ...
        _seed(db, sid="s2", n=5)
        view = db.messages.window_around("s2", ids1[2], window=2)
        assert view["window"] == []
...
class TestScrollPattern:
    ...
    def test_scroll_forward_re_anchored_on_last_id(self, db):
        ids = _seed(db, n=20)
        anchor = ids[5]
        v1 = db.messages.window_around("s1", anchor, window=3)
        last_id = v1["window"][-1]["id"]
        v2 = db.messages.window_around("s1", last_id, window=3)
    ...
    def test_scroll_backward_re_anchored_on_first_id(self, db):
        ...
        v2 = db.messages.window_around("s1", first_id, window=3)
...
class TestContentHydration:
    def test_content_is_decoded(self, db):
        ids = _seed(db, n=3)
        view = db.messages.window_around("s1", ids[1], window=1)
    ...
    def test_tool_calls_deserialized(self, db):
        ...
        db.messages.append("s1", role="assistant", content="", tool_calls=tc_payload)
        mid = db.messages.append("s1", role="tool", content="result", tool_name="x")

        view = db.messages.window_around("s1", mid, window=2)
