...
EXPLICITLY_ALLOWED_WITHOUT_STATIC_WORKER_CALL = {
    ...
    # Runtime stability is discovered dynamically and then invoked through
    # the component proxy, so the AST scanner cannot see these method names.
...
class _WorkerDBCallVisitor(ast.NodeVisitor):
    ...
    def _record_getattr_db_method_call(self, node: ast.Call) -> None:
        ...
        # `participants.<method>`.
        if name_arg.value in {"participants", "runtime_stability"}:
            return
