...
def test_fire_cron_job_scopes_store_and_runtime_home_together(
    ...
    class RecordingProvider:
        def fire_due(self, job_id, *, adapters=None, loop=None):
            ...
            captured["jobs_file"] = cron_jobs.current_cron_store().jobs_file
            return True
...
async def test_cron_profile_scan_runs_off_event_loop(isolated_profiles, monkeypatch):
    from hermes_cli import cron_dashboard_routes, web_server
    ...
    original_call_service = cron_dashboard_routes._call_service

    def tracking_call_service(*args, **kwargs):
        worker_threads.put(threading.get_ident())
        return original_call_service(*args, **kwargs)

    monkeypatch.setattr(
        cron_dashboard_routes,
        "_call_service",
        tracking_call_service,
    )
...
@pytest.mark.asyncio
async def test_cron_run_history_resolves_name_and_isolates_profile(
    isolated_profiles,
):
    ...
    worker_job = web_server._call_cron_for_profile(
        "worker_alpha",
        "create_job",
        prompt="record worker runs",
        schedule="every 1h",
        name="worker-history",
    )
    worker_store = open_cli_session_store(
        isolated_profiles["worker_alpha"] / "state.db"
    )
    default_store = open_cli_session_store(isolated_profiles["default"] / "state.db")
    try:
        worker_session = f"cron_{worker_job['id']}_20260720_120000"
        worker_store.sessions.create(worker_session, "cron")
        worker_store.messages.append(worker_session, "user", "worker run")
        worker_store.sessions.end(worker_session, "completed")

        default_store.sessions.create(worker_session, "cron")
        default_store.messages.append(worker_session, "user", "wrong profile")
        default_store.sessions.end(worker_session, "completed")
    finally:
        worker_store.close()
        default_store.close()

    result = await web_server.list_cron_job_runs(
        "worker-history",
        profile="worker_alpha",
        limit=500,
    )
    ...
    assert [run["id"] for run in result["runs"]] == [worker_session]
    assert result["runs"][0]["preview"] == "worker run"
    assert result["runs"][0]["profile"] == "worker_alpha"
    assert result["runs"][0]["archived"] is False


@pytest.mark.asyncio
async def test_cron_run_history_io_runs_off_event_loop(
    isolated_profiles,
    monkeypatch,
):
    from hermes_cli import cron_dashboard_routes, web_server
    ...
    monkeypatch.setattr(cron_dashboard_routes, "_call_service", tracking_call_service)

    await web_server.list_cron_job_runs("retained-job", profile="worker_alpha")
