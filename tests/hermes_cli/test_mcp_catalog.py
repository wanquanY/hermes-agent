...
def _default_mock_probe(monkeypatch):
    ...
    monkeypatch.setattr(mc, "_plugin_catalog_manifests", lambda: [])
...
class TestFigmaDesktopCatalog:
    """Contract for Hermes's first-party Figma integration policy."""

    def _enable_figma_catalog(self, monkeypatch):
    ...
    def _figma_entry(self, monkeypatch):
        monkeypatch.delenv("HERMES_OPTIONAL_MCPS", raising=False)
        self._enable_figma_catalog(monkeypatch)
        from hermes_cli.mcp_catalog import get_entry

        entry = get_entry("figma-desktop")
        assert entry is not None
        return entry

    def test_ships_local_desktop_endpoint_with_read_first_defaults(self, monkeypatch):
        entry = self._figma_entry(monkeypatch)

        assert entry.transport.type == "http"
        assert entry.transport.url == "http://127.0.0.1:3845/mcp"
        assert entry.auth.type == "none"

        enabled = entry.tools.default_enabled
        ...
        assert not any(name.startswith(("add_", "create_", "update_", "delete_")) for name in enabled)

    def test_remote_allowlisted_endpoint_is_not_advertised_as_installable(self, monkeypatch):
        monkeypatch.delenv("HERMES_OPTIONAL_MCPS", raising=False)
        self._enable_figma_catalog(monkeypatch)
        from hermes_cli.mcp_catalog import list_catalog
        ...
        entries = list_catalog()
        assert not any(
            entry.transport.url == "https://mcp.figma.com/mcp"
            for entry in entries
        )

    def test_noninteractive_install_persists_safe_defaults_when_desktop_is_offline(
        self, monkeypatch
    ):
        entry = self._figma_entry(monkeypatch)
        from hermes_cli.config import load_config
        ...
        server = load_config()["mcp_servers"]["figma-desktop"]
        assert server["url"] == "http://127.0.0.1:3845/mcp"
        assert server["enabled"] is True
        assert server["tools"]["include"] == entry.tools.default_enabled
