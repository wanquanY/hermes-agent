...
def hermes_home(tmp_path, monkeypatch):
    """Isolated HERMES_HOME so shared state writes don't clobber the real one."""
    ...
    # Bust the goal-module's store cache so it re-resolves HERMES_HOME.
...
class TestWaitBarrier:
    ...
    def test_parked_on_live_pid_does_not_continue_or_judge(self, hermes_home):
        ...
        with patch.object(goals, "_pid_alive", return_value=True):
            ...
            mgr.wait_on(self._live_pid, reason="CI green")
    ...
    def test_barrier_auto_clears_when_process_exits_and_loop_resumes(self, hermes_home):
        ...
        mgr.set("ship it", max_turns=5)
        mgr.wait_on(self._live_pid, reason="build")
        with patch.object(goals, "_pid_alive", return_value=True):
            assert mgr.is_waiting() is True

        with patch.object(goals, "_pid_alive", return_value=False):
    ...
    def test_barrier_persists_and_reloads(self, hermes_home):
        ...
        with patch.object(goals, "_pid_alive", return_value=True):
            ...
            mgr.set("g")
            mgr.wait_on(self._live_pid, reason="deploy")

            # Fresh manager loads the persisted barrier.
            mgr2 = GoalManager(session_id="wb-persist")
            assert mgr2.state.waiting_on_pid == self._live_pid
...
class TestJudgeDrivenWait:
    ...
    def test_judge_wait_pid_parks_loop(self, hermes_home):
        ...
        with patch.object(goals, "_pid_alive", return_value=True):
            ...
            with patch.object(
                goals, "judge_goal",
                return_value=("wait", "CI watcher still running", False, {"pid": live_pid}),
            ):
                decision = mgr.evaluate_after_turn(
                    "Pushed the PR, watching CI.",
                    background_processes=[{
                        "pid": live_pid, "command": "wait_for_pr_green.sh",
            ...
            assert decision["should_continue"] is False
            assert decision["continuation_prompt"] is None
            assert mgr.state.waiting_on_pid == live_pid
            assert mgr.is_waiting() is True
