...
@pytest.fixture()
def doxie_agent(monkeypatch, tmp_path):
    ...
    with (
        patch(
            ...
            return_value=_make_tool_defs("doxie_automation_task_create"),
        ),
...
def test_run_conversation_returns_directly_after_automation_create(doxie_agent):
    tool_call = _mock_tool_call(
        "doxie_automation_task_create",
    ...
    doxie_agent.client.chat.completions.create.return_value = _mock_response(
    ...
    with (
        ...
        patch.object(doxie_agent, "_persist_session"),
        patch.object(doxie_agent, "_save_trajectory"),
        patch.object(doxie_agent, "_cleanup_task_resources"),
    ):
        result = doxie_agent.run_conversation("创建一个每天上午十点整理 AI 资讯的定时任务")

    assert doxie_agent.client.chat.completions.create.call_count == 1
    assert result["api_calls"] == 1
    assert result["completed"] is True
    assert result["turn_exit_reason"] == "direct_tool_response(doxie_automation_task_create)"
...
def test_run_conversation_does_not_direct_return_failed_automation_create(doxie_agent):
    tool_call = _mock_tool_call(
        "doxie_automation_task_create",
        call_id="call_create",
    )
    doxie_agent.client.chat.completions.create.side_effect = [
        _mock_response(finish_reason="tool_calls", tool_calls=[tool_call]),
        _mock_response(content="没有可用的 Doxie 会话上下文。", finish_reason="stop"),
    ]
    ...
    with (
        ...
        result = doxie_agent.run_conversation("创建定时任务")

    assert doxie_agent.client.chat.completions.create.call_count == 2
    assert result["final_response"] == "没有可用的 Doxie 会话上下文。"
