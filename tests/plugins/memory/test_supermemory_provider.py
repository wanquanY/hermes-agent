...
def test_base_url_resolution_precedence(monkeypatch):
    monkeypatch.setenv("SUPERMEMORY_BASE_URL", "http://env.example:6767/")

    assert _resolve_base_url("https://config.example/v1/") == (
        "https://config.example/v1"
    )
    assert _resolve_base_url() == "http://env.example:6767"
    monkeypatch.delenv("SUPERMEMORY_BASE_URL")
    assert _resolve_base_url() == "https://api.supermemory.ai"


def test_initialize_routes_fake_client_to_configured_base_url(
    ...
    memory = SupermemoryMemoryProvider()
    memory.initialize("session-local", hermes_home=str(tmp_path), platform="cli")

    assert memory._base_url == "http://localhost:6767"
    assert memory._client.base_url == "http://localhost:6767"


def test_client_passes_normalized_base_url_to_sdk(monkeypatch):
    import sys
    import types
...
def test_conversation_ingest_uses_client_base_url(
