...
flows through, regardless of where it came from:
  ...
  * ``blueprint`` — the user installed a skill that carries a ``blueprint:``
                   block (see ``tools/blueprints.py``); installing it registers a
                   suggestion instead of auto-scheduling.
  * ``recipe``   — legacy source retained for existing installed skills.
...
VALID_SOURCES = frozenset(
    {"catalog", "blueprint", "recipe", "usage", "integration"}
)
