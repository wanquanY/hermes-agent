...
def select_moa_as_default(config: dict[str, Any]) -> None:
    """Select a configured MoA preset as the default virtual provider."""
    ...
    for name in names:
        ...
        aggregator_label = (
            f"{aggregator.get('provider')}:{aggregator.get('model')}"
        )
        suffix = "  ← default" if name == default_name else ""
        rows.append(
            f"{name}  (agg {aggregator_label}, "
            f"{len(preset.get('reference_models') or [])} refs){suffix}"
        )
    selected_index = _prompt_choice(
        "Select a Mixture of Agents preset",
        rows,
        names.index(default_name) if default_name in names else 0,
    )
    ...
    _print_config({"moa": {**moa, "default_preset": selected_name}})
