"""MCP catalog — curated, repository-reviewed MCP servers shipped with Hermes.
...
Catalog policy:
- Entries are added only through repository review. Presence in the
  ``optional-mcps/`` directory means the manifest is part of the shipped
  catalog; there is no live community feed or implicit remote trust tier.
...
See website/docs/user-guide/features/mcp.md for user docs. The parser and data
classes in this module are the canonical manifest schema implementation.
"""
...
def _plugin_catalog_manifests() -> List[Path]:
    """Return MCP manifests owned by currently enabled plugins.

    Import is deliberately lazy: the plugin loader itself must be able to
    register declarative assets without importing this catalog module.
    """
    ...
    try:
        ...
        discover_plugins()
        return get_plugin_manager().list_mcp_catalog_manifests()
    except Exception:
        logger.debug("Could not discover plugin MCP catalog assets", exc_info=True)
        return []
...
def list_catalog() -> List[CatalogEntry]:
    ...
    manifests.extend(_plugin_catalog_manifests())
    ...
    for manifest in manifests:
        try:
            ...
            if existing is not None:
                _CATALOG_DIAGNOSTICS.append(
                    (
                        entry.name,
                        "duplicate",
                        f"duplicate catalog entry ignored: {manifest} conflicts with {existing}",
                    )
                )
                continue
            seen_names[entry.name] = manifest
            ...
            if "manifest_version" in msg and "unsupported" in msg:
                _CATALOG_DIAGNOSTICS.append(
                    (manifest.parent.name, "future_manifest", msg)
                )
            else:
                _CATALOG_DIAGNOSTICS.append((manifest.parent.name, "invalid", msg))
            continue
...
def catalog_diagnostics() -> List[tuple]:
    ...
    is one of:
      ...
      - ``duplicate`` — a later plugin entry tried to shadow an earlier
        repository-reviewed or plugin-owned entry with the same name.
    """
