...
def _register_command_family(
    ...
    *,
    ...
    local_summary_fallbacks = {
        ("tools", "list"): "Show all tools and their enabled/disabled status",
        (
            "slack",
            "manifest",
        ): (
            "Print or write a Slack app manifest with every gateway command "
    ...
    for child_path in paths:
        ...
        command_summary = (
            ...
            or local_summary_fallbacks.get(full_path)
            or f"Run `hermes {usage}`."
        )
        engine.register(
            full_path,
            usage,
...
def _sessions_stats(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    db = open_cli_session_store()
    try:
        total = db.sessions.count(archived="all")
        listable = len(
        ...
        for source in ["cli", "tui", "telegram", "discord", "slack", "cron"]:
            count = db.sessions.count(source=source, archived="all")
            if count:
...
def _sessions_export(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        ...
        try:
            if ns.session_id:
                resolved_session_id = db.sessions.resolve_id(ns.session_id)
                ...
                rows = db.sessions.export_all(source=ns.source)
    ...
    def _run() -> None:
        ...
        try:
            ...
            if not resolved_session_id:
                raise ConsoleCommandError(f"Session '{ns.session_id}' not found.")
            title = " ".join(ns.title)
            if not db.sessions.set_title(resolved_session_id, title):
                raise ConsoleCommandError(f"Session '{ns.session_id}' not found.")
...
def _sessions_optimize(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        ...
        try:
            ...
            print("Optimized session storage.")
        finally:
            db.close()
...
def _sessions_repair(_engine: HermesConsoleEngine, args: list[str]) -> str:
    ...
    def _run() -> None:
        ...
        if not db_path.exists():
            ...
            return
        reason = probe_state_database(db_path)
        if reason is None:
        ...
        if ns.check_only:
            return
        report = repair_state_database(db_path, backup=not ns.no_backup)
        if report.repaired:
            ...
            return
        raise ConsoleCommandError(f"Repair failed: {report.error}")
