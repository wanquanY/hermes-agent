...
def _approve(subsystem: str, rest: List[str], memory_store) -> str:
    ...
    out = [f"Approved {result['applied']} {subsystem} write(s)."]
    if result["failed"]:
        out.append("Failed:")
        out.extend(f"  {item}" for item in result["failed"])
    return "\n".join(out)


def approve_pending(subsystem: str, target: str, *, memory_store=None) -> dict:
    """Apply one staged record (or all records) and return structured data."""
    records = wa.list_pending(subsystem)
    if not records:
        return {
            "applied": 0,
            "failed": [],
            "not_found": True,
            "message": f"No pending {subsystem} writes.",
        }

    if str(target or "").lower() == "all":
        ...
        if not record:
            return {
                ...
                "failed": [],
                "not_found": True,
                "message": f"No pending {subsystem} write with id '{target}'.",
            }
        targets = [record]
    ...
    return {"applied": applied, "failed": failed, "not_found": False, "message": ""}
...
def reject_pending(subsystem: str, target: str) -> dict:
    """Discard one staged record (or all records) and return structured data."""
    if str(target or "").lower() == "all":
        rejected = sum(
            1
            for record in wa.list_pending(subsystem)
            if wa.discard_pending(subsystem, record["id"])
        )
        return {
            "rejected": rejected,
            "not_found": False,
            "message": f"Rejected {rejected} pending {subsystem} write(s).",
        }
    if wa.discard_pending(subsystem, target):
        return {
            "rejected": 1,
            "not_found": False,
            "message": f"Rejected pending {subsystem} write '{target}'.",
        }
    return {
        "rejected": 0,
        "not_found": True,
