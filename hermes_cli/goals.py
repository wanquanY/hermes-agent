...
def load_goal(session_id: str) -> Optional[GoalState]:
    ...
    if store is None:
        return None
    try:
        raw = store.metadata.get(_meta_key(session_id))
...
def save_goal(session_id: str, state: GoalState) -> None:
    ...
    if store is None:
        return
    try:
        store.metadata.set(_meta_key(session_id), state.to_json())
    except Exception as exc:
...
def _pid_alive(pid: int) -> bool:
    ...
    Delegates to ``channels.runtime_status._pid_exists`` — the canonical,
    ...
    try:
        ...
        pass
    # Last-resort fallback for reduced installations.
    try:
