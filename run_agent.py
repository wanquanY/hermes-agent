...
class AIAgent(CreditsRuntimeMixin, StreamWriterAgentMixin):
    ...
    def __init__(
        ...
        init_agent(
            ...
            list_terminals_callback=list_terminals_callback,
            read_terminal_callback=read_terminal_callback,
            write_terminal_callback=write_terminal_callback,
    ...
    def _codex_silent_hang_hint(self, *, model: Optional[str] = None) -> Optional[str]:
        """Return a user-facing hint for known Codex silent-reject patterns.

        Some ChatGPT Codex OAuth models have historically accepted a stream
        connection but emitted no events and no provider error. The timeout
        detectors end that hang; this helper makes the user-visible failure
        actionable when the request shape matches a known backend pattern.
        """
    ...
    def _flush_message_participant_id(
        ...
        *,
        ...
        User messages are not authored by the active agent/member run. For
        assistant/tool rows, fall back to the active RunContext so historical
        replay can keep the same speaker identity after process reload.
        """
        for value in (
            msg.get("participant_id"),
            msg.get("participantId"),
            msg_metadata.get("participant_id"),
            msg_metadata.get("participantId"),
        ):
