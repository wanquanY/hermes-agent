...
def _terminal_excerpt(db: Any, run_id: str) -> dict[str, Any]:
...
def prior_node_attempts(db: Any, mission_id: str, node_id: str, *, limit: int = 5) -> list[dict[str, Any]]:
    ...
    with db._lock:
        rows = db._conn.execute(
            ...
            SELECT b.run_id, b.session_id, b.runtime_session_id, b.role, b.created_at,
             ...
             LIMIT ?
            """,
            (mission_id, node_id, max(1, min(int(limit or 5), 20))),
        ).fetchall()
    result: list[dict[str, Any]] = []
    ...
    for row in rows:
        ...
        terminal = _terminal_excerpt(db, run_id)
        result.append({
            key: value
            for key, value in {
                "run_id": run_id,
                ...
                "summary": cap_text(deliverable.get("summary") or terminal.get("excerpt"), 800),
                "result": text(deliverable.get("result")),
...
def recent_node_events(db: Any, mission_id: str, node_id: str, *, limit: int = 20) -> list[dict[str, Any]]:
    mission_id = text(mission_id)
    node_id = text(node_id)
    ...
    if not callable(getter):
        return []
    try:
        events = getter(mission_id, limit=max(50, min(int(limit or 20) * 5, 500)))
    except Exception:
        return []
    result: list[dict[str, Any]] = []
...
def build_team_mission_worker_context(
    *,
    ...
    if db is not None:
        prior = prior_node_attempts(db, _mission_id(mission), _node_id(node), limit=5)
        if prior:
            sections.append((5, "prior_attempts", ["Prior attempts on this node:", _safe_json(prior, limit=WORKER_CONTEXT_FIELD_MAX_CHARS), ""]))
        recent = recent_node_events(db, _mission_id(mission), _node_id(node), limit=20)
        if recent:
            sections.append((5, "recent_events", ["Recent events:", _safe_json(recent, limit=WORKER_CONTEXT_FIELD_MAX_CHARS), ""]))
    if memory_text:
