...
| `docs/upstream-sync/dovie-runtime-snapshot-YYYYMMDD.md` | 某次 staged/发布节点的本地 Dovie runtime 落地快照 | 每次大批量本地变更提交前新增一篇 |

当前最新快照：

- `docs/upstream-sync/dovie-runtime-snapshot-20260622.md`：`session_index` 侧栏索引、Team Mission 轻量列表/幽灵索引清理、state.db incremental vacuum、image attachments。
- `docs/upstream-sync/dovie-runtime-snapshot-20260616.md`：Dovie Gateway contract、profile/team registry、conversation render snapshot、storage stats、Team Mission workspace/recovery。

## 后续同步检查顺序

1. 先读 `UPSTREAM_SYNC_CHECKLIST.md`，按文件变更范围定位风险模块。
2. 再读最新的 `docs/upstream-sync/dovie-runtime-snapshot-*.md`，确认当前本地 contract 和事实源边界。
...
1. 本次 staged 代码新增或改变了哪些 Dovie-facing contract。
2. 哪些本地边界必须在后续上游同步时保留。
3. 哪些上游能力仍不应直接吸收，因为会破坏 Dovie 作为客户端、Hermes 作为 agent engine 的分层。
4. 本轮最小验证包是什么。
