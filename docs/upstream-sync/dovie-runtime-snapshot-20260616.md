# Dovie Runtime 同步快照 - 2026-06-16

## 结论

当前暂存代码继续强化 Hermes 作为 Dovie agent engine 的本地运行时边界，不是对上游 Desktop、Dashboard OAuth、API server session controls 或 React chat surface 的全量吸收。

后续同步上游时，应把本轮变更按以下六组能力核对，而不是按单个文件逐项猜测：

1. Dovie Gateway contract 和 extension method ownership。
...
### 1. Dovie Gateway contract 升级

- `dovie_extension/manifest.py` 的 contract/version 进入 `2026-06-15`。
...
- `dovie_extension/gateway_methods.py` 明确 Dovie method modules 和 overrides，确保 extracted gateway methods 被 Dovie extension 边界接管，而不是散落在 upstream core 默认注册里。

同步检查重点：

- `tui_gateway/core/method_registration.py` 仍必须加载 extension overrides。
- `gateway.capabilities` 必须能反映 manifest 中的 Dovie required methods。
...
- `hermes_agent_profile_growth.py` 提供 profile growth summary，用于 Dovie profile 侧的成长统计。
- `tui_gateway/methods/profile_registry.py` 和 `tui_gateway/methods/team_registry.py` 暴露 Dovie-facing RPC。

同步检查重点：

- 不要把 profile/team registry 降级成 Dovie 客户端本地状态。
...
- `hermes_team_leader_runtime_context.py` 从 team/mission/conversation state 推导 leader runtime context，避免依赖 Dovie 每次传完整 profile runtime details。
...
- `agent/chat_completion_helpers.py`、`agent/dovie_diagnostics.py` 和 streaming 测试覆盖 Dovie 诊断与流式边界。
...
- Dovie diagnostics 必须保持无锁 stderr 输出，不能引入 logging lock 死锁风险。
...
- `runtime.ensure` 仍由 Dovie extension、system methods、runtime proxy 共同维护。
- 暂存代码收敛 worker 环境变量边界：worker 启动环境保留 `HERMES_HOME`、`DOVIE_HERMES_RUNTIME_SCOPE_KEY`、`DOVIE_AGENT_PROFILE_ID`，不再把 profile version id 当作必需环境变量。
- `tui_gateway/services/prompt_attachments.py` 统一 prompt submit 附件字段解析，Dovie document parse enrichment 继续在 extension 层完成。
- `agent/auxiliary_client.py`、`agent/title_generator.py` 等辅助模型路径继续接受 Dovie runtime credentials/profile context。

同步检查重点：

- scoped control RPC 需要代理到 runtime worker 的必须继续代理，不能误跑在 control plane。
- prompt attachment 只做归一化和 enrichment，不应把 Dovie 文档解析逻辑扩散回 Hermes core。
- 子进程 runtime 的 profile identity 应以 scope/profile id 为主，不依赖易漂移的 profile version env。

## 本轮不吸收的上游方向

- 不用上游完整 Dashboard OAuth / WS ticket 替换 Dovie sidecar token 与 runtime proxy。
- 不用上游 API server session controls 替换 Dovie `run_control` / `prompt.submit` / `runtime.ensure` 主链路。
- 不把 Team Mission 回退成 Kanban SQLite 投影或客户端自建 graph store。
- 不把 Dovie document parse、display transcript sanitization、desktop browser bridge 从 extension 边界拆回 core。
