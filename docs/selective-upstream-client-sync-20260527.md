...
> 文档管理说明：本文保留为 2026-05-27 起 P0/P1 选择性吸收的历史审计，不再继续承载每个 Dovie runtime 发布节点的长篇流水账。后续同步入口见 `docs/upstream-sync/README.zh-CN.md`；当前本地运行时快照见 `docs/upstream-sync/dovie-runtime-snapshot-20260616.md`。
...
- 完整 Dashboard OAuth 登录页面、WS ticket、SPA AuthWidget 和 re-auth envelope：这些需要先和 Dovie sidecar token / runtime worker bridge 的认证关系做设计，不在本轮直接套入。
- API server session controls / media session chat 的完整实现：本轮只吸收能力发现和安全相关部分，避免绕过 Dovie run control 与 prompt attachment contract。
- TUI session orchestrator、Docker/s6、website/i18n、optional skills/MCP 大量新增：仍按 P2/P3 暂缓。

## 现状更新（2026-06-02）

本分支在 `dovie-upstream-sync-20260527` 之后继续保留 Dovie 客户端接入边界，并补齐两项本地 runtime 行为：

- Dovie automation 的 create / update / remove 工具结果现在可以走 direct tool response。`agent/direct_tool_response.py` 只对白名单内的 Dovie 自动化工具、单个 tool call、结构化成功事件生效；`agent/conversation_loop.py` 在工具执行后直接生成最终 assistant 回复并结束本轮，避免为了“已创建/已更新/已删除任务”这种确定性结果再请求模型 follow-up，从而减少延迟和二次流式卡住风险。
- `session.list` 和 `session.most_recent` 现在把 `cron` 与 `tool` 一起视为内部 runtime source。Dovie 自动化任务的用户可见结果应通过 current-session / new-session result binding 投影到目标会话；raw cron execution session 不应作为独立聊天出现在 Dovie 会话侧栏，否则会暴露内部 cron prompt 并造成重复会话。

这两项都不是对上游 API session controls 或原生 `cronjob` 的吸收。它们是本地 Dovie contract 的收敛：自动化任务仍由 `tools/dovie_automation_task_tool.py`、`tui_gateway/services/dovie_cron_jobs.py` 和 Dovie run/event binding 管理，模型不重新获得原生 `cronjob` 工具作为主入口。

## 现状更新（2026-06-04，prodv0.9.0）

本地工作区在 `dovie-upstream-sync-20260602` 之后继续把 Hermes 作为 Dovie 的 agent engine，而不是切换到上游 API server session controls。当前改动集中在生产版客户端运行态可观测性和子 agent 权限边界：

- `delegate_task` 子 agent 的工具继承从 `tools/delegate_tool.py` 拆到 `tools/delegate_tool_access.py`。父 agent 可以通过 `enabled_tools` 把已经解析过的精确工具名传给子 agent，子 agent 只获得父级真实加载且未被阻断的工具；请求 `web` / `search` 等语义别名时，也会先映射到 Dovie 托管 web 工具再和父级工具面求交，避免因 toolset 粒度过粗获得 sibling tools。
- 子 agent 身份和展示名由 `tools/subagent_identity.py` 统一清洗、生成和人类可读化。`delegate_task` 现在把 `delegate_call_id`、`agent_name`、`role`、`context`、`dispatch_message`、tool running/completed 状态以及子 agent output delta 一起投递到 gateway event stream，Dovie 右侧运行面板可以稳定关联一次 delegate tool call 下的子任务。
- `hermes_state_runs.py` 对 token 级 stream delta 做持久化合并，并新增 `list_run_events_filtered()` 与 `compact_run_events()`。这使 `run_events` 既能支撑实时流，又不会因为 message / reasoning / subagent delta 激增而让历史回放和 Dovie hydration 退化。
...
- Dovie sidecar 通过 `DOVIE_SIDECAR_PARENT_PID` 增加父进程 watchdog；runtime proxy 会把该 PID 传给 sidecar。父 runtime 退出或 sidecar 被错误收养时，sidecar 会主动退出，避免本地桌面客户端留下孤儿 WebSocket 进程。
...
这些是 Dovie 客户端 contract 的本地生产化收敛，不是对上游 TUI session orchestrator、API server session controls 或完整 dashboard OAuth / WS ticket 链路的吸收。后续合并上游时必须保留：精确工具面继承、runtime-scoped subagent event API、run event compaction、sidecar 父进程生命周期，以及 Dovie 对 recall / interrupt 的子 agent 完结投影。

## 现状更新（2026-06-05，0.9.2）

本地工作区在 `prodv0.9.0` 之后继续按“手工迁移、保留 Dovie contract”的方式吸收上游 `4feb181eb..96cd37e21` 中的稳定性和安全修复。详细分析见 `UPSTREAM_UPDATE_REPORT_2026-06-05.md`。
...
- P1 Branch / Session / Search：`messages.active` soft-delete、Dovie `session_lineage` branch primitives、`session.branch` 模块化实现、`/undo [N]` 和 `/rewind` prefill 合同、SQL-bounded session-id search、Web session search 的 ID 优先和 compression-lineage 去重。
- P1/P2 局部工具和客户端体验：Dovie visible browser bridge source URL、`system.search`、MCP HTML/non-MCP endpoint preflight、Vision native provider shrink recovery、slash command prefill 在 TUI 和 Web 前端统一。

仍未吸收：上游 React Desktop、Bootstrap installer、完整 Dashboard OAuth / remote gateway auth、Channels UI、Skills 全量 catalog 和 progressive tool disclosure。这些需要另行按 Dovie profile/runtime_scope、云端模型代理和技能市场生命周期设计。

## 现状更新（2026-06-10，Team Mission runtime）

本地工作区当前 staged 代码把 Dovie Team Mission 从产品层 Kanban projection 推进为 Hermes 原生运行时。详细设计和实现快照见 `docs/team-mission-runtime-architecture.zh-CN.md`。
...
这不是对上游 React Desktop / Dashboard / Kanban UI 的吸收。它是 Dovie 客户端 contract 的本地原生化：Hermes 是 Team Mission graph/history/stream 的事实源，Dovie 只消费 Hermes API 和 event stream。

## 现状更新（2026-06-11，prodv0.9.3）

本地工作区当前 staged 代码继续收敛 Team Mission 生产运行边界，目标是让 Dovie 的 Team Mission 详情面板、Leader 工具面和任务图状态都以 Hermes 为事实源。
...
Tag 计划：`prodv0.9.3`。该节点仍不是上游 full merge；它是 Dovie Team Mission runtime 的本地生产化补齐。

## 现状更新（2026-06-13，Dovie runtime diagnostics / terminalization）

本地工作区当前 staged 代码继续修复 Dovie 作为 Hermes agent engine 时的运行态问题：卡 running 不可诊断、sidecar 退出后 active run 残留、Team Mission Leader 会话被普通 profile context 污染、内部 node runtime session 泄露到会话列表。

当前节点包括：

- Dovie diagnostics：新增 `agent/dovie_diagnostics.py`，conversation loop、system prompt、memory prefetch、API kwargs、streaming request、chat-completions client/create、gateway agent build 等关键阶段输出无锁 stderr 诊断。
...
这些改动仍是 Dovie 本地 contract 的生产化，不是吸收上游 Desktop / Dashboard UI。Hermes 继续作为 Team Mission graph/history/stream 和 active run 状态的唯一事实源。
...
结论：能力发现已吸收；session controls / media session chat 未作为 Dovie 客户端主链路替代方案吸收。

| 提交 | 状态 | 当前证据 | 说明 |
|---|---|---|---|
| `25f43d38d` | 已吸收 | `gateway/platforms/api_server.py` 注册 `/v1/skills`、`/v1/toolsets` | 用于客户端能力发现，不替代 Dovie gateway contract |
| `96223265b` | 已吸收 | `gateway/platforms/api_server.py` capability 中包含 `skills_api: true` | capability 与 API 路由一致 |
| `f7527b0fd` | 未作为 P1 直接吸收 | API server 当前已有部分 session capability 字段，但未采纳完整 session controls 作为客户端主控路径 | 继续以 `tui_gateway/services/run_control.py` 为 Dovie run control 边界 |
| `464b51d45` | 未作为 P1 直接吸收 | 当前只保留本地 prompt attachments / document parse contract | 不用上游 media session chat 替换 Dovie attachment enrichment |

### P1 核对：Dashboard / WebSocket 安全

结论：安全基线已吸收；完整 OAuth / WS ticket / SPA 重登录链路未吸收，需另做 Dovie token 与 dashboard auth 的统一设计。
...
| `5b17eab67` | 未吸收 | 无 `/auth/*`、`/login` 完整 gate middleware | 需先设计与 Dovie sidecar token 的关系 |
...
| 能力 | 本地关键路径 | 上游是否等价 |
|---|---|---:|
| Dovie gateway contract / capabilities | `dovie_extension/*`, `tui_gateway/core/method_registration.py` | 否 |
...
| prompt attachments / document parse | `dovie_extension/prompt_attachments.py`, `document_parse_tool.py` | 否 |
| desktop visible browser bridge | `dovie_extension/browser_bridge.py`, `tools/browser_tool.py` | 否 |
| Dovie automation | `tools/dovie_automation_task_tool.py`, `tui_gateway/services/dovie_cron_jobs.py`, `agent/direct_tool_response.py` | 否 |
...
| 提交 | 内容 | 建议方式 | 原因 | 本地风险点 |
|---|---|---|---|---|
| `775a17284` | strip Hermes-internal scaffolding keys before chat.completions | 手工移植 | 防止内部 metadata 进入 provider 请求 | 不能删除 DB/event 中的 Dovie metadata |
| `20b3703a4` | tailor length-continuation prompt for partial stream | 手工移植 | partial stream 后续补全更稳定 | 不能破坏 run terminal event |
| `9140be7c2` | emit `finish_reason=length` on text-only partial-stream stub | 手工移植 | 客户端可识别需要 continuation | 和 Dovie structured delta 合并 |
...
| `b6ca56f65` | recover from `invalid_encrypted_content` | 手工移植 | Responses replay 容错 | 不能重放 foreign Dovie runtime metadata |
...
| `8edeebe6d` | plugin transformed response survives streaming suppression | 手工移植 | 插件改写回复不应被流式路径吞掉 | Dovie extension hook 结果不能丢 |
...
这些不是 Dovie 客户端接入的前置条件，但对客户端动态展示可用能力有价值。

| 提交 | 内容 | 建议方式 | 原因 | 本地风险点 |
|---|---|---|---|---|
| `25f43d38d` | API server `GET /v1/skills` and `/v1/toolsets` | 手工移植设计，不直接照搬 | 客户端可展示技能和工具集 | 必须包含 Dovie extension tools/toolsets |
| `96223265b` | mark `skills_api` capability true | 手工移植 | capability 暴露给客户端 | 不能让能力声明早于 Dovie tools 注册 |
| `f7527b0fd` | API server session controls | 暂不直接吸收，先设计映射 | 有外部 session control 价值 | 不能绕过 Dovie run control |
| `464b51d45` | media in session chat API | 只吸收附件模型思想 | 与 Dovie document attachments 有重叠 | 不能破坏 `parse_document` 提示增强 |

建议设计：

```text
客户端能力发现优先走 Dovie gateway contract。
...
| 客户端需求 | 建议方法 | 数据来源 |
|---|---|---|
| 查询当前可用 skills | `skills.list` 或 API server `/v1/skills` | profile-scoped skills hub + Dovie extension |
| 查询当前可用 toolsets | `toolsets.list` 或 API server `/v1/toolsets` | `toolsets.py` + extension registered toolsets |
| 查询 runtime 能力 | `gateway.capabilities` | Dovie manifest + current worker |
| 提交多媒体/文档 | 继续走 `prompt.submit.attachments` | Dovie attachment enrichment + `parse_document` |
...
这组上游提交对“公网 dashboard / 反代 / OAuth 登录 / WS 认证”有价值。如果当前客户端只走本地 sidecar token，不是立即必需；如果 Dovie 客户端要走 Portal 或公网 dashboard，应进入 P1。

| 提交 | 内容 | 建议方式 | 原因 | 本地风险点 |
|---|---|---|---|---|
| `8773bbf18` | `should_require_auth` predicate | 手工移植 | auth gate 判断可复用 | 不能误伤本地 sidecar |
| `949ad95e4` | stash `auth_required` on app.state | 手工移植 | 前端可知道是否需要登录 | 与 Dovie auth state 合并 |
| `5b17eab67` | auth gate middleware + `/auth/*` + `/login` | 暂缓全量，拆设计 | 功能完整但侵入大 | 可能覆盖 Dovie token auth |
| `53736b392` | gated mode fail-closed / proxy headers | 手工吸收安全策略 | 反代安全 | 本地开发 loopback 不应误拦 |
| `b69fce9c8` | single-use WS tickets + `/api/auth/ws-ticket` | 手工设计性吸收 | WS 鉴权更安全 | Dovie runtime worker bridge 要兼容 |
| `b2360ba44` | ticket auth on all WS endpoints | 手工设计性吸收 | 所有 WS 入口一致 | 不要阻断 sidecar internal WS |
| `8971e9483` | SPA `getWsTicket()` + `buildWsAuthParam()` | 借鉴前端实现 | 客户端 WS 连接安全 | Dovie native client 可能不用 SPA |
| `5e9308b5b` | 401 re-auth envelope + `next=` propagation | 可选 | 前端重登录体验 | native client 需不同错误模型 |
| `2fc4615fc` | AuthWidget + `/api/status` auth fields | 可选 | Web dashboard 展示登录状态 | Dovie 客户端可能自己展示状态 |
| `c3104195b` | bypass loopback WS peer check in gated mode | 手工吸收 | 本地开发可用 | 避免扩大公网例外 |
| `b26d81d53` | `X-Forwarded-Prefix` + secure cookie handling | 可选吸收 | 反代部署需要 | cookie/path 与 Dovie domain 设计相关 |
| `2e66eefbc` | validate WebSocket Host and Origin | 应吸收 | WS 安全基线 | 需要允许 Dovie local origins |
| `973255986` | restrict dashboard websockets to loopback clients | 应吸收但加 Dovie allowlist | 防远程滥用 WS | Dovie desktop/client origin 白名单 |

建议设计：

```text
不要直接把 upstream OAuth gate 套到 Dovie sidecar 上。
先定义三类调用方：
1. local dashboard browser
2. Dovie native/desktop client
...
- 本地 Dovie desktop client 能连接 gateway WS。
...
| 提交 | 内容 | 建议方式 | 原因 | 本地风险点 |
|---|---|---|---|---|
| `5744b1757` | markdown link scheme restriction + `defusedxml` | 手工移植 | 文档/网页/解析安全 | Dovie document parse 要继承 |
...
| `30928f945` | dashboard plugin assets allowlist + env denylist | 手工移植 | dashboard plugin 安全 | Dovie plugin/static assets 要检查 |
| `8bf99227f` | plugin API path traversal + project RCE | 手工移植 | 高风险安全修复 | 不能绕过 project plugin gate |
| `09f85f2cf` | truthy env semantics for project-plugin gate | 手工移植 | gate 行为更明确 | Dovie env bridge 是否设置 |
| `3bace071b` | sensitive store file permissions | 手工移植 | 本地凭据安全 | profile-scoped state 权限 |
| `41d2c758c` | unsafe gateway media path delivery | 手工移植 | media/file 交付安全 | Dovie artifact path 同步校验 |
...
这些对 Hermes Ink TUI 有价值，但不是 Dovie 客户端接入的基础能力。如果 Dovie 客户端主要走自己的 UI + TUI Gateway，则优先级低于 P0/P1。

| 提交 | 内容 | 建议方式 | 原因 | 风险 |
|---|---|---|---|---|
| `0a83247e9` | TUI session orchestrator | 暂缓，先评估 | 多 session 体验好 | 涉及 `tui_gateway/server.py`，侵入大 |
| `50aaf0c4a` | delineate assistant responses from details | 手工吸收思想 | transcript 展示更清晰 | Dovie structured stream 已有 tool events |
| `874c2b1fe` | ignore late thinking deltas after completion | 手工移植 | 防止完成后 UI 被晚到事件污染 | 要和 Dovie event seq 对齐 |
...
本地 Dovie automation 已经替代原生 `cronjob` 工具，这组上游更新只吸收安全/日志部分，不改变 Dovie automation contract。

| 提交 | 内容 | 建议方式 | 原因 | 本地风险点 |
|---|---|---|---|---|
| `d952b377a` | cron API provenance logging | 手工移植 | 审计更完整 | Dovie result binding metadata 要保留 |
| `2c3ca475c` | reject id mutation + validate output paths | 手工移植 | 防误改和路径风险 | Dovie output/artifact path 要纳入 |
| `9863a07af` | layer `agent.disabled_toolsets` onto cron baseline | 手工移植 | cron 继承禁用工具集 | Dovie automation 默认继承当前 runtime |
...
当前客户端接入里最关键的是 Dovie desktop visible browser bridge。上游这次没有等价桥接能力，但有少量通用 browser/vision 修复。

| 提交 | 内容 | 建议方式 | 原因 | 本地风险点 |
|---|---|---|---|---|
| `22f3f5a75` | browser daemon cleanup process-tree termination | 手工移植 | 防 browser daemon 泄漏 | 不要影响 Dovie visible session |
| `3d66787a0` | auxiliary vision provider=openai routing | 手工移植 | vision routing 更正确 | Dovie model descriptor vision 覆盖保留 |
| `83f6a83b2` | TUI images with codex app-server | 可选 | 图片展示体验 | 与 Dovie attachments 关系弱 |
...
| ntfy / platform plugin 新增 | 与 Dovie 客户端接入无直接关系 |
...
provider 请求中不带 Dovie/Hermes 内部 metadata。
...
tui_gateway/dovie_gateway_contract.py
tui_gateway/core/method_registration.py
model_tools.py
toolsets.py
dovie_extension/manifest.py
```
...
Dovie extension tools 和 Dovie toolsets 被包含。
API session control 不绕过 Dovie run control。
```
...
```text
Dovie local client、dashboard browser、runtime worker bridge 三类连接都能通过正确认证。
...
Dovie artifact/document/media path 不允许 traversal。
profile home 下状态/凭据文件权限正确。
```

### 批次 5：Dovie automation / browser 辅助修复
...
```text
Dovie automation 仍默认绑定当前会话。
原生 cronjob 不重新暴露给模型。
Dovie desktop browser 可用时 navigate/snapshot/action 仍优先走 visible session。
vision routing 仍遵守 Dovie model descriptor。
```
...
| `hermes_cli/web_server.py` | upstream dashboard auth 和 Dovie sidecar auth 需要重新设计边界 |
| `gateway/platforms/api_server.py` | upstream API session control 可能绕过 Dovie run control |
| `toolsets.py` | 本地 Dovie toolset / cronjob 替换语义必须保留 |
...
只有满足“文件冲突小、语义不碰 Dovie contract”的提交才考虑直接 cherry-pick：
...
1. 启动 Dovie 客户端，调用 `gateway.capabilities`，确认 Dovie required methods 全部存在。
...
5. 上传文档附件，确认 prompt 被 Dovie attachment context 增强，模型能调用 `parse_document`。
6. 调用 `session.list`、`workspace.list`、`artifacts.list`，确认只读查询不会创建空 DB。
7. Dovie desktop browser 可用时，`browser_navigate` / `browser_snapshot` / `browser_click` 结果带 `provider=dovie_desktop`。
