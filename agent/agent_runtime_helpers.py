...
def refresh_reasoning_config(agent, model: str | None = None) -> bool:
    """Refresh reasoning policy after a runtime model transition."""
    try:
        from hermes_cli.config import load_config
        from hermes_constants import resolve_reasoning_config

        effective_model = str(model or getattr(agent, "model", "") or "")
        agent.reasoning_config = resolve_reasoning_config(
            load_config() or {},
            effective_model,
        )
        logger.info(
            "Reasoning config resolved for model %s: %s",
            effective_model,
        ...
        logger.debug(
            "Could not refresh reasoning config for model %s: %s",
            model or getattr(agent, "model", ""),
            exc,
        )
        return False
...
def try_recover_primary_transport(
    ...
    try:
        ...
        refresh_reasoning_config(agent, rt["model"])
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            ...
            preset = resolve_moa_preset(
                _load_moa_config().get("moa") or {}, eff_model or None
            )
            ...
            if aggregator_provider and aggregator_model:
            ...
            logger.debug(
                "MoA aggregator cache-policy resolution failed",
                exc_info=True,
            )
        return False, False
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    if (new_provider or "").strip().lower() == "moa":
        ...
        try:
            ...
            logger.debug("custom-provider TLS settings skipped during model switch", exc_info=True)
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    """Forward tool invocation to the unified middleware-aware boundary."""
