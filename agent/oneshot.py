...
A one-shot is a stateless model call that runs outside conversation history.
Surfaces use it for small generative chores where an agent turn would pollute
the transcript, while named templates keep prompt design consistent.
"""
...
def run_oneshot(
    *,
    ...
    """Run one stateless LLM request and return plain text."""
    if template:
...
def _strip_code_fence(text: str) -> str:
    """Drop one wrapping Markdown code fence when a model adds it."""
