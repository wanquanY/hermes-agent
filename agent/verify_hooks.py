...
def max_verify_nudges(config: Optional[dict[str, Any]] = None) -> int:
    ...
    verification_cfg = agent_cfg.get("verification")
    raw = (
        verification_cfg.get("plugin_max_attempts")
        if isinstance(verification_cfg, dict)
        and "plugin_max_attempts" in verification_cfg
        else agent_cfg.get("max_verify_nudges")
    )
    try:
...
def coding_verify_guidance(config: Optional[dict[str, Any]] = None) -> Optional[str]:
    ...
    verification_cfg = agent_cfg.get("verification")
    enabled = (
        verification_cfg.get("guidance", True)
        if isinstance(verification_cfg, dict)
        else agent_cfg.get("verify_guidance", True)
    )
