...
def sanitize_replay_history(
    ...
    *,
    ...
    """Apply every model-replay sanitizer in the canonical order.

    Interrupted blocks are resolved first, then dangling calls. Callers at a
    persisted-session boundary also pass ``now`` to expire destructive-action
    confirmations before any new user input is interpreted.
    """
    if not agent_history:
        return agent_history
    cleaned = strip_dangling_tool_call_tail(
