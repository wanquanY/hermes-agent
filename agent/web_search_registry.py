...
def _resolve(configured: Optional[str], *, capability: str) -> Optional[WebSearchProvider]:
    """Resolve the active provider for search, extract, or crawl.
...
def _disabled_web_plugin_for(configured: Optional[str] = None, *, capability: Optional[str] = None) -> Optional[str]:
    ...
    Pass ``capability`` ("search" | "extract" | "crawl") to resolve the configured
    ...
    if not configured and capability in ("search", "extract", "crawl"):
        configured = (
...
def get_active_crawl_provider() -> Optional[WebSearchProvider]:
    ...
    fallback) from config.yaml. Callers should expect ``None`` when no
    registered provider advertises native crawl support.
    """
