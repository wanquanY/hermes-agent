...
COMPUTER_USE_GUIDANCE = (
    ...
    "time.\n\n"
    "## Preferred workflow\n"
    "1. If multiple displays, multiple windows, or an ambiguous app may be "
    "involved, call `computer_use` with `action='list_targets'` first and "
    "choose the exact `target_id`/window/display. Otherwise call "
    "`computer_use` with `action='capture'` and `mode='som'` "
    "(default). You get a screenshot with numbered overlays on every "
    "interactable element plus an AX-tree index listing role, label, and "
    "bounds for each numbered element.\n"
    "2. Keep using the same `target_id` for follow-up captures when one was "
    "returned. Click by element index: `action='click', element=14`. This is "
    ...
    "as target binding, not proof that the visible frontmost app changed.\n"
