...
from hermes_agent.application.storage_maintenance_service import StorageMaintenanceService
from hermes_agent.application.team_capability_service import TeamCapabilityService
from hermes_agent.application.team_mission_audit_log import TeamMissionAuditLog
from hermes_agent.application.team_mission_audit_service import TeamMissionAuditService
...
class CliSessionStore(TeamMissionStateMixin):
    ...
    def db_path(self) -> Path:
        """Return the immutable connection target without touching SQLite.

        Disconnect checkpoint persistence may still be completing while the
        owner starts shutdown. Reading connection metadata after ``close`` is
        both unnecessary and unsafe across threads in CPython's sqlite layer.
        """
