...
from hermes_agent.storage.state_mixins.participants import (
    ParticipantsMixin,
...
class HermesStateStore(AgentProfileStateMixin, TeamRegistryStateMixin, TeamCapabilityStateMixin, TeamMissionStateMixin, MemberChatStateMixin, ParticipantsMixin, ActivitiesMixin, RunStateMixin, BranchStateMixin):
    """
    ...
    def reconcile_conversation_participants_one_shot(self) -> Dict[str, Any]:
        ...
        This is a one-shot data migration with an idempotent repair path: once
        marked complete it normally skips, but it still runs if participant rows
        are clearly below the conversation count. Existing participant rows are
        never overwritten.
        """
        ...
        def _do(conn: sqlite3.Connection) -> Dict[str, Any]:
            ...
            participant_rows = int(
                conn.execute("SELECT COUNT(*) AS count FROM conversation_participants").fetchone()["count"]
                or 0
            )
            session_rows = int(conn.execute("SELECT COUNT(*) AS count FROM sessions").fetchone()["count"] or 0)
            team_conversation_rows = int(
                conn.execute("SELECT COUNT(*) AS count FROM team_mission_conversations").fetchone()["count"]
                or 0
            )
    ...
    def _merge_message_metadata(current: Any, patch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key, value in patch.items():
            if isinstance(value, dict) and isinstance(base.get(key), dict):
                base[key] = HermesStateStore._merge_message_metadata(base[key], value)
            else:
                base[key] = value
        return base
    ...
    def _team_main_transcript_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for message in messages:
            if not isinstance(message, dict) or not is_main_transcript_message(message):
                continue
            stable_id = HermesStateStore._team_transcript_stable_message_id(message)
            if stable_id:
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        This migration is deliberately not part of automatic HermesStateStore startup
    ...
    def enable_telegram_topic_mode(
        ...
        *,
        ...
        HermesStateStore startup must not create these side tables.
        """
...
HermesStateStore.try_acquire_compression_lock = _backfilled_try_acquire_compression_lock
HermesStateStore.release_compression_lock = _backfilled_release_compression_lock
