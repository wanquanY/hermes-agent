"""Dovie agent profile design bridge tools.

The Hermes tool exposes the product-level 分身设计 workflow. Models first inspect
real local catalog data, then emit structured draft requests. Dovie owns
...
def _backend_call(command: str, payload: dict | None = None) -> Any:
    ...
    ok, raw = run_blocking_interruptibly(
        _send,
        interrupted_message="Dovie backend bridge request interrupted",
    )
    if not ok:
        detail = raw.decode("utf-8", errors="replace")
        raise RuntimeError(f"Dovie backend bridge rejected {command}: {detail}")
...
def _draft_targets_profile(draft_id: str, target_agent_profile_id: str) -> bool:
    if not draft_id or not target_agent_profile_id or not _backend_bridge_available():
        return False
    try:
        draft = _backend_call("dovie_agent_profile_draft_get", {"draftId": draft_id})
    except Exception:
        return False
...
DESIGN_AGENT_PROFILE_SCHEMA = {
    ...
    "description": (
        "Design a Dovie agent profile draft through the product-level 分身设计 workflow. "
        "Use this when the user asks to create or revise a persona/agent/profile/分身. "
        "First call operation=inspect_context to get real Dovie templates, "
        ...
        "tools or skills. Dovie will open the right-side creation workspace for "
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "draft_id": {
                "type": "string",
                "description": "Existing Dovie draft id to update. Leave empty only when creating a brand-new agent profile draft.",
            },
            "draft_reference": {
                "type": "string",
            ...
            "architecture_template_id": {
                "type": "string",
                "description": "Optional Dovie architecture template id used to shape this profile.",
            },
...
TEST_AGENT_PROFILE_SCHEMA = {
    ...
    "description": (
        "Synchronously test a Dovie draft agent profile by sending a free-form "
    ...
    "parameters": {
        ...
        "properties": {
            "draft_id": {
                "type": "string",
                "description": "Optional Dovie draft id to test. If omitted, Dovie resolves the active or matching draft from the current design conversation.",
            },
            "draft_reference": {
                "type": "string",
...
def _avatar_assets() -> list[dict]:
    styles = [
        ...
        ("bottts", ["Hermes", "Dovie", "Gateway", "Builder", "Operator", "Research"]),
...
def _list_dovie_skill_categories() -> list[dict]:
    try:
        from tools.dovie_skill_categories import dovie_runtime_mode_enabled, list_dovie_skill_categories
        if not dovie_runtime_mode_enabled():
            return []
        return list_dovie_skill_categories()
    except Exception as exc:
...
def _profile_design_context_event(
    ...
    skill_categories = _list_dovie_skill_categories()
    ...
    result = {
        ...
        "catalogQueries": {
            ...
            "skill_categories": "Call catalog_kind='skill_categories' to list the Dovie admin skill categories that skill_manage(create).category must use.",
...
def _profile_design_event(**kwargs) -> dict:
    ...
    return {
        "dovie_event": "agent_profile_design_draft_requested",
        "operation": operation,
        "draft": {
...
def _profile_design_backend_event(**kwargs) -> dict:
    ...
    if should_update_active_draft:
        ...
        draft = _backend_call("dovie_agent_profile_draft_update", payload)
        ...
        draft = _backend_call("dovie_agent_profile_draft_create_revision", payload)
        normalized_operation = "revision"
    else:
        payload.pop("draftId", None)
        draft = _backend_call("dovie_agent_profile_draft_create", payload)
        normalized_operation = "create"
    return {
        "dovie_event": "agent_profile_design_draft_saved",
...
def _install_skill_to_draft_event(**kwargs) -> dict:
    ...
    if not _backend_bridge_available():
        raise RuntimeError("Dovie backend bridge is required to install a skill into a draft profile.")

    prepared = _backend_call(
        "dovie_agent_profile_draft_prepare_runtime",
        _contextual_payload(
            {
    ...
    if not isinstance(prepared, dict) or not prepared.get("prepared"):
        return {
            "dovie_event": "agent_profile_draft_skill_install_resolution_required",
    ...
    updated_draft = _backend_call("dovie_agent_profile_draft_update", update_payload)
    return {
        "dovie_event": "agent_profile_draft_skill_installed",
        "draftId": draft_id,
...
def design_agent_profile(**kwargs) -> str:
    ...
    if operation == "install_skill":
        try:
            ...
            return tool_error(f"failed to install skill into Dovie draft profile: {exc}")
    if _backend_bridge_available():
        try:
            return tool_result(_profile_design_backend_event(**kwargs))
        except Exception as exc:
            return tool_error(f"failed to save Dovie agent profile draft: {exc}")
...
def list_agent_profile_drafts(**kwargs) -> str:
    return _backend_tool_result("dovie_agent_profile_draft_list", kwargs)


def get_agent_profile_draft(**kwargs) -> str:
    return _backend_tool_result("dovie_agent_profile_draft_get", kwargs)


def resolve_agent_profile_draft(**kwargs) -> str:
    return _backend_tool_result("dovie_agent_profile_draft_resolve", kwargs)


def list_agent_profiles(**kwargs) -> str:
    return _backend_tool_result("dovie_agent_profile_list", kwargs)


def get_agent_profile(**kwargs) -> str:
    return _backend_tool_result("dovie_agent_profile_resolve", kwargs)


def create_agent_profile_revision_draft(**kwargs) -> str:
    try:
        draft = _backend_call("dovie_agent_profile_draft_create_revision", _contextual_payload(kwargs))
        return tool_result(
            {
                "dovie_event": "agent_profile_design_draft_saved",
                "operation": "revision",
...
def prepare_agent_profile_draft_runtime(**kwargs) -> str:
    return _backend_tool_result("dovie_agent_profile_draft_prepare_runtime", kwargs)
...
def _draft_context(home: Path, draft_id: str, expectation: str = "") -> str:
    ...
    parts = [
        f"You are being tested as Dovie draft agent profile `{draft_id}`.",
...
def test_agent_profile(
    ...
    if _backend_bridge_available():
        ...
        if not isinstance(prepared, dict) or not prepared.get("prepared"):
            ...
            payload = {
                "dovie_event": "agent_profile_test_resolution_required",
                **payload,
        ...
        if not normalized_draft_id:
            return tool_error("draft_id is required when the Dovie backend bridge is unavailable.")
        home = _draft_home(normalized_draft_id)
    if not home.is_dir():
        return tool_error(
            f"Draft runtime is not prepared: {normalized_draft_id}. "
            "Dovie could not prepare its transient runtime files."
        )
    ...
    try:
        raw = delegate_task(
            ...
            # delegate_task does not inherit the parent turn's dovie/skills tools.
            toolsets=selected_toolsets or ["__dovie_no_tools__"],
...
for _name, _description, _handler, _properties in [
    (
        ...
        "List Dovie agent profile drafts visible to the current design conversation. Use before testing when draft_id is unknown.",
        list_agent_profile_drafts,
        {
    ...
    (
        ...
        "Get a Dovie agent profile draft by id.",
        get_agent_profile_draft,
        {"draftId": {"type": "string"}},
    ),
    (
        "resolve_agent_profile_draft",
        "Resolve the active or matching Dovie draft from the current design conversation.",
        resolve_agent_profile_draft,
        {"reference": {"type": "string"}, "draftId": {"type": "string"}},
    ),
    (
        "list_agent_profiles",
        "List published Dovie agent profiles.",
        list_agent_profiles,
        {"query": {"type": "string"}},
    ),
    (
        "get_agent_profile",
        "Get a published Dovie agent profile by id or slug.",
        get_agent_profile,
        {"agentProfileId": {"type": "string"}, "slug": {"type": "string"}},
    ),
    (
        "create_agent_profile_revision_draft",
        "Create an editable revision draft from the latest published Dovie agent profile.",
        create_agent_profile_revision_draft,
        {
    ...
    (
        ...
        "Prepare the transient Hermes runtime files for a Dovie draft profile.",
        prepare_agent_profile_draft_runtime,
        {"draftId": {"type": "string"}, "reference": {"type": "string"}},
    ),
    (
        "install_skill_to_agent_profile_draft",
        "Install an existing Hermes skill into a Dovie draft profile runtime scope, then add it to the draft recommendedSkills. Use after creating a reusable skill with skill_manage and confirming it should belong to the draft.",
        install_skill_to_agent_profile_draft,
        {
            "draftId": {"type": "string"},
