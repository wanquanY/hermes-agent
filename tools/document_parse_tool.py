#!/usr/bin/env python3
"""Document parsing tool for local Hermes attachments.

The main Doxie backend already parses uploaded documents through
FileParserService. Hermes runs as a separate local runtime, so it needs the
same capability as a native tool instead of asking the model to shell out to
pdftotext, python snippets, or cat. This module keeps the parser self-contained
and uses optional document libraries when present, with standard-library
fallbacks for Office Open XML documents.
"""
...
except Exception:  # pragma: no cover - fallback for unusual import order
...
class MinerUError(RuntimeError):
    """Raised when MinerU parsing is unavailable or fails."""
...
def _is_text_document(ext: str, mime_type: str) -> bool:
    ...
    return normalized_mime.startswith(TEXT_MIME_PREFIXES) or normalized_mime in TEXT_MIME_TYPES
...
def _parse_with_doxie_mineru_proxy(
    *,
    ...
    if normalized_url:
        ...
        logger.info(
            "parse_document: using Doxie MinerU proxy for URL document "
            "file_type=%s file_name=%s",
            file_type,
            fields["file_name"],
        )
    else:
        ...
        logger.info(
            "parse_document: using Doxie MinerU proxy for local document "
            "file_type=%s file_name=%s bytes=%s",
            file_type,
            path.name,
            path.stat().st_size,
        )
    ...
    logger.info(
        "parse_document: Doxie MinerU proxy succeeded parser=%s file_name=%s chars=%s",
        result.get("parser") or "mineru",
        fields.get("file_name") or "document",
    ...
    for table in body.findall("w:tbl", ns):
        ...
        for row in table.findall(".//w:tr", ns):
            ...
            for cell in row.findall(".//w:tc", ns):
                cell_text = " ".join(t.strip() for t in _xml_texts(cell, "t") if t.strip())
...
def parse_document_tool(
    ...
    if ext in MINERU_SUPPORTED_EXTENSIONS:
        try:
            ...
            logger.warning(
                "parse_document: Doxie MinerU proxy failed for file_type=%s file_name=%s; "
                "allow_local_fallback=%s error=%s",
                ext,
    ...
    logger.info(
        "parse_document: using local parser parser=%s file_type=%s file_name=%s%s",
        parsed.get("parser") or "unknown",
        ext,
        resolved.name,
        f" mineru_fallback_reason={mineru_error}" if mineru_error else "",
    )
...
DOCUMENT_PARSE_SCHEMA = {
    "name": "parse_document",
    "description": (
        "Parse an uploaded/local document into text. Use this for PDFs, PowerPoint, "
        "Word, Excel, CSV, JSON, XML, Markdown, and TXT attachments before "
        "answering questions about their contents. PDF/PPT/PPTX documents are "
        "parsed through the Doxie-managed MinerU proxy when available, so MinerU "
        "tokens stay on the Doxie backend. "
        "Plain text-like documents are read directly; Office documents use local parsers. "
        "Use this instead of terminal commands such as cat, pdftotext, or ad-hoc "
        "Python snippets for attachment understanding."
    ),
    "parameters": {
        "type": "object",
        "properties": {
            "path": {
                "type": "string",
                "description": "Optional absolute, relative, or ~/ path to the local document attachment. Used for local fallback and exact text/code reads.",
            },
            "file_url": {
                "type": "string",
                "description": "Optional cloud/public HTTP(S) URL for MinerU parsing. When omitted, PDF/PPT/PPTX local path files are uploaded to MinerU by this tool.",
            },
            "file_type": {
                "type": "string",
            ...
            "mime_type": {
                "type": "string",
                "description": "Optional MIME type override from the uploaded attachment metadata.",
            },
            "offset": {
                "type": "integer",
                "description": "Character offset for paginating extracted text. Default: 0.",
                "default": 0,
                "minimum": 0,
            },
            "max_chars": {
            ...
            "allow_local_fallback": {
                "type": "boolean",
                "description": "Allow local parser fallback if MinerU fails or is unavailable. Default: true.",
                "default": True,
            },
        },
        "required": [],
