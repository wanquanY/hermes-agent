...
def _pending_path(subsystem: str, pending_id: str) -> Path:
    normalized = str(pending_id or "").strip().lower()
    if not _PENDING_ID_RE.fullmatch(normalized):
        raise ValueError("pending id must be an 8-character lowercase hex id")
    return _pending_dir(subsystem) / f"{normalized}.json"
...
def stage_write(subsystem: str, payload: Dict[str, Any],
    ...
    Returns a dict with ``id`` and metadata. Staging is a durability boundary:
    disk failures raise ``RuntimeError`` so callers can truthfully report that
    no pending record was created instead of claiming a lost write was staged.
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"failed to durably stage pending {subsystem} write"
        ) from e
    return record
