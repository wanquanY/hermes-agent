...
DOVIE_AUTOMATION_CREATE_SCHEMA = {
    "type": "function",
    "function": {
        "name": "dovie_automation_task_create",
        "description": (
            "Create a Dovie automation task bound to the current conversation's "
            "agent profile, runtime scope, and workspace. Use this instead of "
            "asking the user to choose an agent or workdir inside a Dovie chat."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                ...
                "schedule": {
                    ...
                    "description": "When to run. Accepts Dovie schedule objects or Hermes cron strings.",
                },
...
DOVIE_AUTOMATION_LIST_SCHEMA = {
    "type": "function",
    "function": {
        "name": "dovie_automation_task_list",
        "description": (
            "List Dovie automation tasks owned by the current conversation context. "
...
DOVIE_AUTOMATION_UPDATE_SCHEMA = {
    "type": "function",
    "function": {
        "name": "dovie_automation_task_update",
        "description": (
            "Edit a Dovie automation task owned by the current conversation. "
            "By default this only edits tasks bound to the current conversation session."
        ),
        "parameters": {
            "type": "object",
            "properties": {
                "task_id": {"type": "string", "description": "Task ID from dovie_automation_task_list."},
...
DOVIE_AUTOMATION_REMOVE_SCHEMA = {
    "type": "function",
    "function": {
        "name": "dovie_automation_task_remove",
        "description": (
            "Remove a Dovie automation task owned by the current conversation. "
...
def _has_dovie_product_context() -> bool:
    return bool(_session_design_context())


def _dovie_automation_tool_available() -> bool:
    # Tool schema resolution happens before per-turn Dovie product context is
    # installed. Do not gate visibility on HERMES_DOVIE_PRODUCT_CONTEXT here;
...
def _job_owner(job: dict[str, Any]) -> dict[str, Any]:
    ...
    dovie = raw.get("dovie") if isinstance(raw.get("dovie"), dict) else {}
    raw_owner = dovie.get("owner") if isinstance(dovie.get("owner"), dict) else {}
...
def _job_result_binding(job: dict[str, Any]) -> dict[str, Any]:
    ...
    raw = job.get("raw") if isinstance(job.get("raw"), dict) else {}
    dovie = raw.get("dovie") if isinstance(raw.get("dovie"), dict) else {}
    raw_binding = dovie.get("result_binding") if isinstance(dovie.get("result_binding"), dict) else {}
...
def _result_binding_payload(mode: str | None, context: dict[str, Any]) -> dict[str, Any] | None:
    ...
    if normalized == "current-session":
        ...
        if not session_id:
            raise ValueError("current-session result binding requires the active Dovie conversation session.")
...
def dovie_automation_task_create(
    *,
    name: str,
    prompt: str,
    schedule: Any,
    ...
    if not owner.get("agentProfileId"):
        return tool_error("Dovie automation tasks require an active Dovie agent profile context.")
    ...
    if binding_mode == "current-session":
        ...
        if not session_id:
            return tool_error("current-session result binding requires the active Dovie conversation session.")
...
def dovie_automation_task_list(
    *,
    ...
    if not _current_agent_profile_id(context):
        return tool_error("Dovie automation task list requires an active Dovie agent profile context.")
    jobs = _list_visible_jobs(context, scope=scope, include_disabled=include_disabled)
    return tool_result({
        "dovie_event": "automation_job_listed",
...
def dovie_automation_task_update(
    *,
    task_id: str,
    scope: str = "current-session",
    name: str | None = None,
    ...
    context = _session_design_context()
    if not _current_agent_profile_id(context):
        return tool_error("Dovie automation task update requires an active Dovie agent profile context.")
    ...
    from tui_gateway.services.dovie_cron_jobs import update_cron_job

    job = update_cron_job({"id": resolved["id"], "patch": patch})
    return tool_result({
        "dovie_event": "automation_job_updated",
        "job": job,
    })


def dovie_automation_task_remove(
    *,
    task_id: str,
    scope: str = "current-session",
) -> str:
    context = _session_design_context()
    if not _current_agent_profile_id(context):
        return tool_error("Dovie automation task remove requires an active Dovie agent profile context.")
    resolved = _resolve_visible_job(context, task_id, scope=scope)
    if isinstance(resolved, str):
        return tool_error(resolved)

    from tui_gateway.services.dovie_cron_jobs import remove_cron_job
...
registry.register(
    name="dovie_automation_task_create",
    toolset="cronjob",
    schema=DOVIE_AUTOMATION_CREATE_SCHEMA,
    handler=lambda args, **kw: dovie_automation_task_create(**args),
    check_fn=_dovie_automation_tool_available,
    emoji="clock",
...
registry.register(
    name="dovie_automation_task_list",
    toolset="cronjob",
    schema=DOVIE_AUTOMATION_LIST_SCHEMA,
    handler=lambda args, **kw: dovie_automation_task_list(**args),
    check_fn=_dovie_automation_tool_available,
    emoji="clock",
    max_result_size_chars=32_000,
)
...
registry.register(
    name="dovie_automation_task_update",
    toolset="cronjob",
    schema=DOVIE_AUTOMATION_UPDATE_SCHEMA,
    handler=lambda args, **kw: dovie_automation_task_update(**args),
    check_fn=_dovie_automation_tool_available,
    emoji="clock",
    max_result_size_chars=32_000,
)
...
registry.register(
    name="dovie_automation_task_remove",
    toolset="cronjob",
    schema=DOVIE_AUTOMATION_REMOVE_SCHEMA,
    handler=lambda args, **kw: dovie_automation_task_remove(**args),
    check_fn=_dovie_automation_tool_available,
    emoji="clock",
    max_result_size_chars=32_000,
)
