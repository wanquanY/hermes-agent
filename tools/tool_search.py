...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    Dovie and Team Mission tools are also first-party runtime contracts, not
    optional plugin affordances. They must remain directly visible so a
    profile-scoped or mission-scoped run never depends on a discovery bridge
    for lifecycle, handoff, document, or product actions.
    """
    ...
    try:
        ...
        if entry is None:
            return False
        toolset = str(entry.toolset or "").strip()
        if toolset.startswith("mcp-"):
            return True
        if toolset.startswith(("dovie", "team_mission")):
            return False
