...
* :func:`import_fal_client` — lazy import + ``lazy_deps`` integration so
  ``fal_client`` is not pulled at cold start.
...
def import_fal_client() -> Any:
    """Import ``fal_client`` through lazy dependency installation when available."""
    try:
    ...
    import fal_client  # type: ignore  # noqa: WPS433
...
def _extract_http_status(exc: BaseException) -> Optional[int]:
    """Return an HTTP status code from httpx/FAL exceptions when one is exposed."""
...
class _ManagedFalSyncClient:
    """Adapter for driving ``fal_client.SyncClient`` against a managed queue host."""
