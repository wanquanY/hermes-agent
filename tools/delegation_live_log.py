...
# view, not the full-fidelity record — the child's durable transcript and
...
def wrap_stream_callback(inner_cb, writer: LiveTranscriptWriter):
    """Tee assistant stream deltas into the live log.

    The delegated-agent runtime exposes assistant output on a dedicated
    callback, separate from tool progress. ``None`` is the end-of-stream
    sentinel and must flush the buffered transcript line.
    """
    ...
    def _cb(delta):
        try:
            if delta is None:
                ...
                writer.add_stream_delta(str(delta))
        except Exception as exc:  # pragma: no cover - writer is best-effort
            logger.debug("Live transcript stream write failed: %s", exc)
        if inner_cb is not None:
            inner_cb(delta)
...
def wrap_reasoning_callback(inner_cb, writer: LiveTranscriptWriter):
    """Tee provider reasoning deltas into the live log."""

    def _cb(delta):
        try:
            if delta:
                writer.thinking(str(delta))
        except Exception as exc:  # pragma: no cover - writer is best-effort
            logger.debug("Live transcript reasoning write failed: %s", exc)
        if inner_cb is not None:
            inner_cb(delta)
...
def attach_live_transcript_callbacks(child: Any, writer: Any) -> None:
    """Attach all live-log callback tees to one constructed child agent.

    The operation is idempotent for the same writer. Keeping this seam public
    lets the orchestrator enforce the contract even when child construction is
    replaced by a test double or a future provider-specific factory.
    """
    if writer is None or getattr(child, "_live_transcript_writer", None) is writer:
        return
    child.tool_progress_callback = wrap_progress_callback(
        getattr(child, "tool_progress_callback", None),
        writer,
    )
    child.stream_delta_callback = wrap_stream_callback(
        getattr(child, "stream_delta_callback", None),
        writer,
    )
    child.reasoning_callback = wrap_reasoning_callback(
        getattr(child, "reasoning_callback", None),
        writer,
    )
    child._live_transcript_writer = writer
    child._live_transcript_path = str(writer.path) if writer.path is not None else None
...
def create_live_transcripts(
    ...
    try:
        ...
        paths_by_index: List[Optional[str]] = []
        for i, t in enumerate(task_list):
            ...
            paths_by_index.append(str(w.path) if w.path is not None else None)
        paths = [path for path in paths_by_index if path is not None]
        if not paths:
            return None, [None] * n, []
        _write_manifest(deleg_id, task_list, paths_by_index)
