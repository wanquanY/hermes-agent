"""Doxie skill category catalog helpers.

Doxie owns the product taxonomy for skill categories. Hermes still owns local
skill files, so Doxie runtimes expose the enabled category catalog through a
...
def doxie_runtime_mode_enabled() -> bool:
    return bool(
        str(os.getenv("DOXIE_HERMES_RUNTIME_MODE") or "").strip()
        or str(os.getenv("DOXIE_SKILL_CATEGORIES_URL") or "").strip()
        or str(os.getenv("DOXIE_API_ORIGIN") or "").strip()
    )
...
def _categories_url() -> str:
    explicit = str(os.getenv("DOXIE_SKILL_CATEGORIES_URL") or "").strip()
    if explicit:
        return explicit
    origin = str(os.getenv("DOXIE_API_ORIGIN") or "").strip().rstrip("/")
...
def _fetch_categories() -> list[dict[str, Any]]:
    ...
    if not url:
        raise RuntimeError("DOXIE_SKILL_CATEGORIES_URL is not configured.")
    if not token:
        raise RuntimeError("DOXIE_LLM_RUNTIME_TOKEN is not configured.")
    ...
    try:
        ...
        raise RuntimeError(f"Doxie skill category request failed: {exc}") from exc

    try:
        payload = json.loads(raw.decode("utf-8") or "{}")
    except Exception as exc:
        raise RuntimeError(f"Doxie skill category response is not JSON: HTTP {status}") from exc
...
def list_doxie_skill_categories(*, force_refresh: bool = False) -> list[dict[str, Any]]:
...
def validate_doxie_skill_category(category: str | None) -> str | None:
    if not doxie_runtime_mode_enabled():
        return None
    normalized = str(category or "").strip()
    try:
        categories = list_doxie_skill_categories()
    except Exception as exc:
        return f"Doxie skill categories are unavailable: {exc}"

    allowed_slugs = [item["slug"] for item in categories]
    if not allowed_slugs:
        return "Doxie skill categories are empty. Create an enabled skill category in Admin first."
    if not normalized:
        return "category is required in Doxie runtimes. Choose one of: " + ", ".join(allowed_slugs)
    if normalized not in set(allowed_slugs):
        return f"Invalid Doxie skill category '{normalized}'. Choose one of: " + ", ".join(allowed_slugs)
    return None
