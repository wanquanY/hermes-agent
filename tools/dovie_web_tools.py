"""Dovie-managed SERPER search and webpage parsing tools.

These tools keep paid web/search credentials on the Dovie backend. Packaged
...
DOVIE_WEB_PROXY_DEFAULT_TIMEOUT_SECONDS = 45
DOVIE_WEB_PROXY_MAX_TIMEOUT_SECONDS = 60


SERPER_SEARCH_SCHEMA = {
    "name": "serper_search_tool",
    "description": (
        "Use Dovie-managed SERPER/Google Search. Supports region, language, "
...
JINA_WEB_PARSER_SCHEMA = {
    ...
    "description": (
        "Parse a webpage into clean Markdown or text through the Dovie-managed "
...
def _bounded_timeout(*values: Any) -> int:
    ...
    requested = candidates[0] if candidates else DOVIE_WEB_PROXY_DEFAULT_TIMEOUT_SECONDS
    return max(5, min(requested, DOVIE_WEB_PROXY_MAX_TIMEOUT_SECONDS))
...
def _post_json(url: str, payload: dict[str, Any], *, token: str, timeout: int) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f"Dovie web proxy request timed out after {timeout}s") from exc
    except socket.timeout as exc:
        raise RuntimeError(f"Dovie web proxy request timed out after {timeout}s") from exc
    except urllib.error.URLError as exc:
        raise RuntimeError(f"Dovie web proxy request failed: {exc}") from exc

    try:
        result = json.loads(raw.decode("utf-8"))
    except Exception as exc:
        raise RuntimeError(f"Dovie web proxy returned non-JSON response: HTTP {status}") from exc
...
def _proxy_result(env_name: str, payload: dict[str, Any]) -> str:
    ...
    timeout = _bounded_timeout(
        payload.get("timeout"),
        _env_int("DOVIE_WEB_PROXY_TIMEOUT", DOVIE_WEB_PROXY_DEFAULT_TIMEOUT_SECONDS),
    )
    try:
        ...
        result = run_blocking_interruptibly(
            ...
            interrupted_message="Dovie web proxy request interrupted",
        )
    except Exception as exc:
...
def serper_search_tool(args: dict[str, Any]) -> str:
    ...
    return _proxy_result("DOVIE_SERPER_PROXY_URL", payload)
...
def jina_web_parser_tool(args: dict[str, Any]) -> str:
    ...
    payload = {
        ...
        "timeout": args.get("timeout", DOVIE_WEB_PROXY_DEFAULT_TIMEOUT_SECONDS),
    }
    return _proxy_result("DOVIE_WEB_PARSE_PROXY_URL", payload)
