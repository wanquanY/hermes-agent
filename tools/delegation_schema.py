...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "Only the final summary enters your context window. Intermediate work "
        "is written to redacted, append-only live transcript files whose paths "
        "are returned with the dispatch/result.\n\n"
        ...
        "- Recurring or scheduled work -> use cronjob (action='create'). For a "
        "durable one-off child run, use mode='async'; its Activity survives the "
        "parent turn and can be observed or cancelled independently.\n\n"
        "IMPORTANT:\n"
        ...
        "- Sync results are returned as an array, one entry per task. Async "
        "mode returns a durable Activity handle immediately; both surfaces "
        "include live_transcripts paths when transcript creation succeeds."
    )
